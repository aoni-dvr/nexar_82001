...
LINUX_MAKE_FLAGS = \
	...
	DEPMOD=$(HOST_DIR)/sbin/depmod LOCALVERSION=
...
ifeq ($(BR2_LINUX_KERNEL_OOSB),y)
LINUX_KERNEL_OOSB =$(LINUX_OVERRIDE_SRCDIR) O=
KERNEL_OOSB_ARCH_PATH = $(LINUX_OVERRIDE_SRCDIR)/arch/$(KERNEL_ARCH)
else
LINUX_KERNEL_OOSB =
KERNEL_OOSB_ARCH_PATH = $(LINUX_ARCH_PATH)
endif
...
define LINUX_BUILD_DTB
	$(LINUX_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) -C $(LINUX_KERNEL_OOSB)$(@D) $(LINUX_DTBS)
endef
...
define LINUX_BUILD_CMDS
	...
	$(foreach dts,$(call qstrip,$(BR2_LINUX_KERNEL_CUSTOM_DTS_PATH)), \
		cp -f $(dts) $(KERNEL_OOSB_ARCH_PATH)/boot/dts/
	)
	$(LINUX_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) -C $(LINUX_KERNEL_OOSB)$(@D) all
	$(LINUX_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) -C $(LINUX_KERNEL_OOSB)$(@D) $(LINUX_TARGET_NAME)
	$(LINUX_BUILD_DTB)
	$(LINUX_APPEND_DTB)
endef
...
define LINUX_INSTALL_TARGET_CMDS
	...
	@if grep -q "CONFIG_MODULES=y" $(@D)/.config; then \
		$(LINUX_MAKE_ENV) $(MAKE1) $(LINUX_MAKE_FLAGS) -C $(LINUX_KERNEL_OOSB)$(@D) modules_install; \
...
linux-rebuild-with-initramfs:
	...
	# Build the kernel.
	$(LINUX_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) -C $(LINUX_KERNEL_OOSB)$(LINUX_DIR) $(LINUX_TARGET_NAME)
	$(LINUX_APPEND_DTB)
