...
#if 1   // Patch from Ambarella, Download and final speed are different.
        if ((index & 0x80) && (baud_chg == 0)) {
            /* Config file use 115200 as final speed.
               So the other packets will use 115200. */
            if (set_speed(dd, ti, 115200) < 0) {
                RS_ERR("Can't set baud rate:115200");
                return -1;
            }
...
static void rtk_write_btmac2file(char bt_addr[6])
{
    ...
    fd = open(BT_ADDR_FILE, O_CREAT | O_RDWR | O_TRUNC, 0777);
...
#ifdef USE_CUSTUMER_ADDRESS
#if 0 // RTK's original setting
    sprintf(bt_config_file_name, BT_ADDR_FILE);
#else // Patches from Ambarella
    snprintf(bt_config_file_name, PATH_MAX, "%s\0", get_macfile());
#endif
...
{
#if 0 // Patch from Ambarella, set speed at last command
