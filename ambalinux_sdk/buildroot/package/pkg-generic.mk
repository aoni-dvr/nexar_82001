...
# Check prebuilt package
$(BUILD_DIR)/%/.stamp_prebuilt:
	@$(call step_start,prebuilt)
	@$(call MESSAGE,"Checking prebuilt packages")
	$(Q)mkdir -p $(@D)
	$(Q)if [ "$(PKG_VERSION)" != "" ]; then \
		if [ -e $(PKG_PBT) ] || [ -e $(PKG_PBH) ]; then \
			touch $(@D)/.stamp_downloaded && sleep 0.01; \
			touch $(@D)/.stamp_extracted && sleep 0.01; \
			touch $(@D)/.stamp_patched && sleep 0.01; \
			touch $(@D)/.stamp_configured && sleep 0.01; \
			touch $(@D)/.stamp_built && sleep 0.01; \
			if [ -e $(PKG_PBH) ]; then \
				echo -e "\tGet prebuilt package -- $(PKG_PBH)"; \
				tar -jxf $(PKG_PBH) -C $(HOST_DIR); \
				touch $(@D)/.stamp_host_installed && sleep 0.01; \
			fi; \
			if [ -e $(PKG_PBS) ]; then \
				echo -e "\tGet prebuilt package -- $(PKG_PBS)"; \
				tar -jxf $(PKG_PBS) -C $(STAGING_DIR); \
				touch $(@D)/.stamp_staging_installed && sleep 0.01; \
			fi; \
			if [ -e $(PKG_PBT) ]; then \
				echo -e "\tGet prebuilt package -- $(PKG_PBT)"; \
				tar -jxf $(PKG_PBT) -C $(TARGET_DIR); \
				touch $(@D)/.stamp_target_installed; \
			fi; \
		fi; \
	fi
	$(Q)touch $@
	@$(call step_end,prebuilt)
...
$(BUILD_DIR)/%/.stamp_rsynced:
	...
	$(Q)if [ "$(PKG)" == "LINUX" ] && [ "$(BR2_LINUX_KERNEL_OOSB)" == "y" ]; then \
		mkdir -p $(@D); \
		rsync -au --chmod=u=rwX,go=rX $($(PKG)_OVERRIDE_SRCDIR_RSYNC_EXCLUSIONS) $(RSYNC_VCS_EXCLUSIONS) $(call qstrip,$(SRCDIR))/tools $(@D); \
		for f in $(LINUX_LICENSE_FILES); do \
			rsync -au -R --chmod=u=rwX,go=rX $($(PKG)_OVERRIDE_SRCDIR_RSYNC_EXCLUSIONS) $(RSYNC_VCS_EXCLUSIONS) $(call qstrip,$(SRCDIR))/./$${f} $(@D); \
		done; \
	elif [ "$($(PKG)_SITE_METHOD)" == "local" ] && [ "$($(PKG)_SITE_OOSB)" == "y" ]; then \
		mkdir -p $(@D); \
	else \
...
$(BUILD_DIR)/%/.stamp_host_installed:
	$(Q)if [ "$(PREBUILT)" == "y" ]; then touch $(HOST_DIR)/.ref_time_$(PKG); fi
	...
	$(Q)if [ "$(PREBUILT)" == "y" ] && [ "$(PKG_VERSION)" != "" ]; then \
		$(call MESSAGE,"Pack $(PKG_PBH)"); \
		rm -f $(PKG_PBH); \
		cd $(HOST_DIR) && \
			mkdir -p $(dir $(PKG_PBH)) && \
			(find -cnewer $(HOST_DIR)/.ref_time_$(PKG) \( -type f -or -type l \) > ../file2pack_$(PKG).txt) && \
			(cat ../file2pack_$(PKG).txt | xargs tar -jcf $(PKG_PBH)) && \
			(rm -f ../file2pack_$(PKG).txt); \
		rm -f $(HOST_DIR)/.ref_time_$(PKG); \
	fi
...
$(BUILD_DIR)/%/.stamp_staging_installed:
	$(Q)if [ "$(PREBUILT)" == "y" ]; then touch $(STAGING_DIR)/.ref_time_$(PKG); fi
	...
	$(Q)if [ "$(PREBUILT)" == "y" ] && [ "$(PKG_VERSION)" != "" ]; then \
		$(call MESSAGE,"Pack $(PKG_PBS)"); \
		rm -f $(PKG_PBS); \
		cd $(STAGING_DIR) && \
			mkdir -p $(dir $(PKG_PBS)) && \
			(find -cnewer $(STAGING_DIR)/.ref_time_$(PKG) \( -type f -or -type l \) > ../file2pack_$(PKG).txt) && \
			(cat ../file2pack_$(PKG).txt | xargs tar -jcf $(PKG_PBS)) && \
			(rm -f ../file2pack_$(PKG).txt); \
		rm -f $(STAGING_DIR)/.ref_time_$(PKG); \
	fi
...
$(BUILD_DIR)/%/.stamp_target_installed:
	$(Q)if [ "$(PREBUILT)" == "y" ]; then touch $(BASE_DIR)/.ref_time_$(PKG); fi
	...
	$(Q)if [ "$(PREBUILT)" == "y" ] && [ "$(PKG_VERSION)" != "" ]; then \
		$(call MESSAGE,"Pack $(PKG_PBT)"); \
		rm -f $(PKG_PBT); \
		cd $(TARGET_DIR) && \
			mkdir -p $(dir $(PKG_PBT)) && \
			(find -cnewer $(BASE_DIR)/.ref_time_$(PKG) \( -type f -or -type l \) > ../file2pack_$(PKG).txt) && \
			(cat ../file2pack_$(PKG).txt | xargs tar -jcf $(PKG_PBT)) && \
			(rm -f ../file2pack_$(PKG).txt); \
		rm -f $(BASE_DIR)/.ref_time_$(PKG); \
	fi
...
$(2)_HASH_FILE = \
	$$(strip \
		$$(if $$(wildcard $$($(2)_PKGDIR)/$$($(2)_VERSION)/$$($(2)_RAWNAME).hash),\
			...
			$(if $(BR2_AMBARELLA_$(2)_VERSION),\
				$$($(2)_PKGDIR)/$(call LOWERCASE,$(2))-$(strip $(subst ",,$(BR2_AMBARELLA_$(2)_VERSION)))/$$($(2)_RAWNAME).hash,\
...
$(2)_TARGET_PREBUILT =		$$($(2)_DIR)/.stamp_prebuilt
...
PREBUILT_PATH = $(DL_DIR)/prebuilt/arm_eabihf
else ifeq ($(BR2_ARM_EABI),y)
PREBUILT_PATH = $(DL_DIR)/prebuilt/arm_eabi
else ifeq ($(BR2_aarch64),y)
PREBUILT_PATH = $(DL_DIR)/prebuilt/arm_aarch64
else
PREBUILT_PATH = $(DL_DIR)/prebuilt/unknown
endif
...
$$($(2)_TARGET_INSTALL_TARGET):		PREBUILT=$(BR2_GEN_PREBUILT)
$$($(2)_TARGET_INSTALL_TARGET):		PKG_PBT=$(PREBUILT_PATH)/pbt_$$($(2)_NAME)-$$($(2)_VERSION).tar.bz2
$$($(2)_TARGET_INSTALL_TARGET):		PKG_VERSION=$$($(2)_VERSION)
$$($(2)_TARGET_INSTALL_STAGING):	PKG=$(2)
$$($(2)_TARGET_INSTALL_STAGING):	PREBUILT=$(BR2_GEN_PREBUILT)
$$($(2)_TARGET_INSTALL_STAGING):	PKG_PBS=$(PREBUILT_PATH)/pbs_$$($(2)_NAME)-$$($(2)_VERSION).tar.bz2
$$($(2)_TARGET_INSTALL_STAGING):	PKG_VERSION=$$($(2)_VERSION)
$$($(2)_TARGET_INSTALL_IMAGES):		PKG=$(2)
$$($(2)_TARGET_INSTALL_HOST):		PKG=$(2)
$$($(2)_TARGET_INSTALL_HOST):		PREBUILT=NO
$$($(2)_TARGET_INSTALL_HOST):		PKG_PBH=$(PREBUILT_PATH)/pbh_$$($(2)_NAME)-$$($(2)_VERSION).tar.bz2
$$($(2)_TARGET_INSTALL_HOST):		PKG_VERSION=$$($(2)_VERSION)
...
$$($(2)_TARGET_PREBUILT):		PKG=$(2)
$$($(2)_TARGET_PREBUILT):		PKG_PBT=$(PREBUILT_PATH)/pbt_$$($(2)_NAME)-$$($(2)_VERSION).tar.bz2
$$($(2)_TARGET_PREBUILT):		PKG_PBS=$(PREBUILT_PATH)/pbs_$$($(2)_NAME)-$$($(2)_VERSION).tar.bz2
$$($(2)_TARGET_PREBUILT):		PKG_PBH=$(PREBUILT_PATH)/pbh_$$($(2)_NAME)-$$($(2)_VERSION).tar.bz2
$$($(2)_TARGET_PREBUILT):		PKG_VERSION=$$($(2)_VERSION)
