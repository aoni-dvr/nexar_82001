...
uchar hci_write_enable_wbs_param[] =
	{ 0x01, 0x7E, 0xFC, 0x03, 0x00, 0x02, 0x00 };
...
/* cf. linux/drivers/tty/serial/ambarella_uart.c::serial_ambarella_set_termios() for Ambarella patch. */
...
int
parse_wbs(char *optarg)
{
	int param;
	int ret;
	int i;

	ret = sscanf(optarg, "%d", &param);
    ...
    hci_write_enable_wbs_param[4] = param;
...
usage(char *argv0)
{
	...
	printf("\t<--wbs=enabled wbs\n");
	printf("\twhere 1 is enable wbs and 0 is disable wbs>\n");
...
parse_cmd_line(int argc, char **argv)
{
	...
	PFI parse[] = { parse_patchram, parse_baudrate,
		...
		parse_scopcm, parse_i2s, parse_no2bytes, parse_tosleep, parse_wbs};
	...
	if (optind < argc) {
		...
		//printf ("Re-open UART: module will detect CTS and re-select transport mode\n");
		usleep(100000);
		close(uart_fd);
		if ((uart_fd = open(argv[optind], O_RDWR | O_NOCTTY)) == -1) {
			fprintf(stderr, "port %s could not be opened, error %d\n",
...
expired(int sig)
{
	...
	retry++;
	if (retry >= 3) {
		fprintf(stderr, "Caution: read will need 30 seconds after exit() called \n");
		daemon(0, 1);
		exit(2);
	} else if (retry) {
...
proc_patchram()
{
	...
	/* use_baudrate_for_download
	 *  Original: use 115200 for download if not set
	 *  AMBA: always use '--baudrate' to download
	 */
	//if (use_baudrate_for_download) {
		//printf ("sleep for bcm43340 in H2\n");
		usleep(100000);
		cfsetospeed(&termios, B115200);
		cfsetispeed(&termios, B115200);
		tcsetattr(uart_fd, TCSANOW, &termios);
	...
	printf("Downloaded, hci reset\n");
	//printf ("sleep for bcm43438a1 in a12\n");
	usleep(100000);
	proc_reset();
}
...
proc_baudrate()
{
	...
	//printf ("sleep for bcm43340 in H2\n");
	usleep(100000);
	if (termios_baudrate == B750000) {
		/* For Ambarella platform. */
		termios.c_ispeed = 750000;
		termios.c_ospeed = 750000;
		termios.c_cflag &= ~(CBAUD | CBAUDEX);
		termios.c_cflag |= B750000;
	} else {
...
proc_wbs()
{
	hci_send_cmd(hci_write_enable_wbs_param,
		sizeof(hci_write_enable_wbs_param));
...
main (int argc, char **argv)
{
	...
	/* use_baudrate_for_download
	 *  Original: runtime always use '--baudrate'
	 *  AMBA: runtime always 115200 if '--use_baudrate_for_download'
	 */
	if (!use_baudrate_for_download) {
		if (termios_baudrate) {
			proc_baudrate();
