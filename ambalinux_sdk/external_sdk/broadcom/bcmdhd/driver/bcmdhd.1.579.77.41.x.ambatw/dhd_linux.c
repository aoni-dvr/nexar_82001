...
#define AMBA_FID	(200)
/* for wlan0 fast resume, the wow filter needs to be deleted at driver */
void ambarella_wow_hook (void)
{
	int pkt_filter_id = AMBA_FID;
	dhd_info_t *dhd;
	struct net_device *dev;
	extern struct net init_net;
	wl_ioctl_t ioc;
	char iovbuf[22];
	memset(&iovbuf, 0x0, sizeof(iovbuf));

	// find wlan0
	dev = first_net_device(&init_net);
	while (dev && strcmp(dev->name, "wlan0")) {
	...
	// do ioctl to delte filter id = AMBA_FID (hard-coded)
	ioc.cmd = WLC_SET_VAR;
	ioc.buf = iovbuf;
	ioc.len = bcm_mkiovar("pkt_filter_delete", (char *)&pkt_filter_id, 4, iovbuf, sizeof(iovbuf));
	ioc.set = TRUE;

	DHD_OS_WAKE_LOCK(&dhd->pub);
	dhd_wl_ioctl(&dhd->pub, 0, &ioc, iovbuf, ioc.len);
