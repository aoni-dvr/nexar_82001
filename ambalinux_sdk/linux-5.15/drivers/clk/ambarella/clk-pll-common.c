			...
			char *prop_name, unsigned long rate)
{
	...
	if (val_num % reg_num) {
		pr_err("%s: please use same elements number "
		       "in amb,clk-regmap and %s\n", np->name, prop_name);
		rval = -1;
		goto END;
	}
...
unsigned long ambarella_pll_calc_vco(struct amb_clk_pll *clk_pll, unsigned long parent_rate)
{
	...
	if (ctrl_val & CTRL_FRAC_MODE) {
		...
		frac = (parent_rate / pre_scaler * frac_val * sdiv) * vcodiv * fsdiv >> 32;
	}
...
unsigned long ambarella_pll_recalc_rate(struct clk_hw *hw, unsigned long parent_rate)
{
	...
	if (ctrl2_val & CTRL2_BYPASS_HSDIV)
		...
		rate = fvco / vcodiv / fsout / sout / post_scaler;

	return rate / clk_pll->fix_divider;
}
		...
		const struct amb_pll_soc_data *soc_data)
{
	...
	if (num < HDMI_CLK_CTRL_OFFSET) {
		pr_err("%s: clk-regmap elements number %d too small\n",
		       np->name, num);
		return;
	}

	if (of_property_read_u32_array(np, "amb,clk-regmap",
				       clk_pll->reg_offset, num) < 0) {
