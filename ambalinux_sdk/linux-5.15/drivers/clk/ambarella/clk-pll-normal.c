			...
			unsigned long parent_rate)
{
	...
	u32 *reg = clk_pll->reg_offset, ctrl_val, frac_val;
	...
	rate_tmp = rate;
	max_numerator = min(128UL, soc_data->vco_max_mhz / (REF_CLK_FREQ / 1000000UL));
	max_denominator = 16;
	...
	while (parent_rate / 1000000 * intp * sdiv / pre_scaler < soc_data->vco_min_mhz) {
		if (sout > 8 || intp > 64)
			break;
		intp += intp_tmp;
		sout += sout_tmp;
	...
	if (clk_pll->frac_mode) {
		...
		if (diff) {
			...
			rct_regmap_en(map, reg[CTRL_OFFSET], ctrl_val | CTRL_FRAC_MODE);
		}
	}
	...
	if (rate_org >= 3000000000UL) {
		...
		ambarella_pll_set_ctrl2(clk_pll, 0);
		ambarella_pll_set_ctrl3(clk_pll, parent_rate);
	}
