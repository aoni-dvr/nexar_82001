...
#if defined(CONFIG_ATF_SUSPEND_SRAM)
static bool psci_fast_resume = 0;
static uintptr_t sram_base = 0;
static uint32_t sram_code_size = 0, sram_stack_size = 0;
#endif
...
static int __psci_cpu_on(u32 fn, unsigned long cpuid, unsigned long entry_point)
{
	...
	if (psci_fast_resume) {
		err = invoke_psci_fn(fn, cpuid, entry_point, booted);
		booted = 1;
	} else {
...
#if defined(CONFIG_ATF_SUSPEND_SRAM)
static int psci_system_wake(unsigned long unused)
{
    struct platform_fast_resume_code *pScratchpad = (struct platform_fast_resume_code *) sram_base;
    return invoke_psci_fn(PSCI_FN_NATIVE(1_0, SYSTEM_WAKE), (unsigned long) pScratchpad->idle2wake_code, 0, 0);
}

static void psci_system_wake_enter(void)
{
    cpu_suspend(0, psci_system_wake);
}
#endif

static int psci_system_suspend_enter(suspend_state_t state)
{
#if defined(CONFIG_ARCH_AMBARELLA_AMBALINK) && defined(CONFIG_AMBALINK_LINKCTRL)
...
#if defined(CONFIG_ATF_SUSPEND_SRAM)
		suspend_set_addr(sram_base, sram_code_size, sram_stack_size);
#endif
	}
}
...
#if defined(CONFIG_ATF_SUSPEND_SRAM)
	if (of_property_read_string(np, "amb,fast_resume", &fast_resume) == 0) {
		if (!strcmp(fast_resume, "ok")) {
			psci_fast_resume = 1;

			if (of_property_read_u32_array(np, "sram-base", base, 2) != 0) {
				pr_err("missing sram_base\n");
            } else {
                sram_base = base[0];
                sram_base = (sram_base << 32) | base[1];
            }
			if (of_property_read_u32(np, "sram-code-size", &sram_code_size) != 0) {
				pr_err("missing sram_code_size\n");
			}
			if (of_property_read_u32(np, "sram-stack-size", &sram_stack_size) != 0) {
				pr_err("missing sram_stack_size\n");
			}
		}
    }
#endif
