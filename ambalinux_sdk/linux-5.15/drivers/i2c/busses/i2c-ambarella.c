...
#define AMBARELLA_I2C_CHANNEL_MAX		(6)
static u32 param_clk_limit_array[AMBARELLA_I2C_CHANNEL_MAX] = {0};
static u32 param_clk_limit_num = AMBARELLA_I2C_CHANNEL_MAX;
module_param_array(param_clk_limit_array, uint, &param_clk_limit_num, S_IRUGO | S_IWUSR);
#endif
	...
	int num)
{
...
#if defined(CONFIG_ARCH_AMBARELLA_AMBALINK) || defined(CONFIG_ARCH_AMBA_CAMERA)
		if ((param_clk_limit_array[adap->nr] != 0U) && (pinfo->clk_limit != param_clk_limit_array[adap->nr])) {
			pinfo->clk_limit = param_clk_limit_array[adap->nr];
	...
	if (errorCode) {
		if (pinfo->state == AMBA_I2C_STATE_NO_ACK) {
			...
			dev_err(
#endif
				pinfo->dev, "No ACK from address 0x%x, %d:%d!\n",
