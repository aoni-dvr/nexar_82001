...
#define AMBARELLA_CALIB_T2V_OFFSET	85000000
...
static int ambarella_adc_probe(struct platform_device *pdev)
{
	...
	ambadc->rct_regmap = syscon_regmap_lookup_by_phandle(np, "amb,rct-regmap");
	if (IS_ERR(ambadc->rct_regmap)) {
		dev_err(&pdev->dev, "no rct regmap!\n");
		return PTR_ERR(ambadc->rct_regmap);
	}

	rval = of_property_read_u32_array(np, "amb,rct-regmap",
			ambadc->rct_offset, ARRAY_SIZE(ambadc->rct_offset));
	if (rval < 0) {
		dev_err(&pdev->dev, "failed to get rct offset!\n");
		return rval;
	}
