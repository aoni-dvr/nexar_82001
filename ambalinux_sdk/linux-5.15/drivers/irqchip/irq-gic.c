...
#if 0 // irq_startup() has been patched.
...
static const struct irq_chip gic_chip = {
	...
	/* SKIP IRQCHIP_SET_TYPE_MASKED:	Mask before calling chip.irq_set_type() */
	/* SKIP IRQCHIP_MASK_ON_SUSPEND:	Mask non wake irqs in the suspend path */
#else
...
static void gic_dist_init(struct gic_chip_data *gic)
{
	...
	if (0 != readl_relaxed(base + GIC_DIST_CTRL)) {
		/* ambalink v1: GIC already enable by rtos */
		ambalink_shm_layout.ambalink_version = 1;
		return;
	} else {
		/* ambalink v2: linux boot before rtos */
		ambalink_shm_layout.ambalink_version = 2;
	}
#endif
...
void gic_dist_restore(struct gic_chip_data *gic)
{
	...
	void __iomem *dist_base;
#ifdef CONFIG_ARCH_AMBARELLA_AMBALINK
	u32 rtos_value, linux_owned_bits, linux_value, int_no, val;
#endif
	...
	/* Interrupt Configuration Registers: do not change RTOS Level/Edge setting */
	for (i = 0; i < DIV_ROUND_UP(gic_irqs, 16); i++) {
		/* 2 bits per interrupt */
		linux_owned_bits = 0;
		for (int_no = (i * 16); int_no < (i + 16); int_no++) {
			val = (((gic_resume_mask[i / 2] >> int_no) & 0x1) ? 0x3 : 0x0);
			linux_owned_bits |= val << ((int_no % 16) << 1);
		}

		/* read current rtos value and clear linux owned Level/Edge setting */
		rtos_value = (readl_relaxed(dist_base + GIC_DIST_CONFIG + i * 4) & ~linux_owned_bits);

		/* set linux owned Level/Edge setting */
		linux_value = rtos_value | (gic->saved_spi_conf[i] & linux_owned_bits);
		writel_relaxed(linux_value,
...
#if defined(CONFIG_ARCH_AMBARELLA_AMBALINK)
	/* Interrupt Priority: set by RTOS */
#else
	for (i = 0; i < DIV_ROUND_UP(gic_irqs, 4); i++)
...
#if defined(CONFIG_ARCH_AMBARELLA_AMBALINK)
	/* Interrupt Processor Targets: set by each driver .resume */
#else
	for (i = 0; i < DIV_ROUND_UP(gic_irqs, 4); i++)
...
#if defined(CONFIG_ARCH_AMBARELLA_AMBALINK)
	/* Interrupt Set-Enable: only enable linux */
	/* Interrupt Clear-Enable: do not clean rtos */
	for (i = 0; i < DIV_ROUND_UP(gic_irqs, 32); i++) {
...
#if defined(CONFIG_ARCH_AMBARELLA_AMBALINK)
	/* Interrupt Set-Active: set by rtos */
	/* Interrupt Clear-Active: do not clean rtos */
#else
	for (i = 0; i < DIV_ROUND_UP(gic_irqs, 32); i++) {
		writel_relaxed(GICD_INT_EN_CLR_X32,
...
#if defined(CONFIG_ARCH_AMBARELLA_AMBALINK)
	/* Distributor Control Register: saved_gicd_ctrl */
