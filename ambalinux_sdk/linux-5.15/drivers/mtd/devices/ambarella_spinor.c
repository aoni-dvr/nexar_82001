...
#if defined(CONFIG_ARCH_AMBARELLA_AMBALINK)
/*
 * Define SPI-NOR Tx/Rx DMA_req in Scratchpad
 */
#ifdef CONFIG_ARCH_AMBARELLA_S5L
#define DMA_REQ_TX (4)
#define DMA_REQ_RX (5)
	...
	defined(CONFIG_ARCH_AMBARELLA_S6LM) || \
	defined(CONFIG_ARCH_AMBARELLA_CV2FS)
#define DMA_REQ_TX (5)
#define DMA_REQ_RX (6)
#else
#error "Undefined CHIP_REV for SPI-NOR"
#endif
...
static void spinor_global_request(struct amb_norflash *flash)
{
	...
	/*spinor.c always polling, amb_nor_spi_irq_handler() will clear REG3C and cannot be polled*/
 ...
 */
static inline int set_4byte(struct amb_norflash *flash, u32 jedec_id,
			    int enable)
{
	int ret = 0;
	...
	default:
		...
		return ambspi_send_cmd(flash, flash->command[0], 0,
				       flash->command[1], 1);
	}
}
		     ...
		     size_t * retlen, u_char * buf)
{
	...
	for (offset = 0; offset < len;) {
		...
		if (needread >= AMBA_SPINOR_DMA_BUFF_SIZE) {
			ret =
			    ambspi_read_data(flash, from + offset,
			...
			memcpy(buf + offset, flash->dmabuf,
			       AMBA_SPINOR_DMA_BUFF_SIZE);
		...
		*retlen = needwrite;
		for (j = needwrite; j < len; j += needcopy) {
			...
			for (i = 0; i < needcopy; i += page_write) {
				...
				write_enable(flash);
				ambspi_prog_data(flash, i, to + i + j,
						 page_write);
				*retlen += page_write;
			}
		}
...
static const struct ambid_t amb_ids[] = {
	{"s25fl256s",
	 INFO(0x010219, 0x4d00, 256 * 1024, 128, 512, 70000000,
	      (DUMMY_4 << 16) | DIO_READ)},
	{"s70fl01gs",
	 INFO(0x010220, 0x4d00, 256 * 1024, 256, 512, 50000000, 0)},
	{"n25q256a",
	 INFO(0x20ba19, 0, 64 * 1024, 512, 256, 60000000,
	      (DUMMY_8 << 16) | Q_READ)},
	{"mt25qu512abb",
	 INFO(0x20bb20, 0, 64 * 1024, 1024, 256, 96000000,
	      (DUMMY_8 << 16) | Q_READ)},
	{"mx25l25645g", INFO(0xc22019, 0, 64 * 1024, 512, 256, 50000000, 0)},
	{"mx66l1g45g",
	 INFO(0xc2201b, 0, 32 * 1024, 4096, 256, 60000000,
	      (DUMMY_4 << 16) | DIO_READ)},
	...
	{"Micron MT25QL01GBBB_1Gb",
	 INFO(0x20ba21, 0, 128 * 1024, 1024, 256, 96000000,
	      (DUMMY_8 << 16) | Q_READ)},
	{},
};
...
{
	struct amb_norflash *flash = dev_data;
	unsigned int irq_status;

	irq_status = readl_relaxed(flash->regbase + REG38);
	writel_relaxed(irq_status, flash->regbase + REG3C);

	return IRQ_HANDLED;
}
#endif

static int amb_get_resource(struct amb_norflash *flash,
...
static int amb_spi_nor_probe(struct platform_device *pdev)
{
...
#if defined(CONFIG_ARCH_AMBARELLA_AMBALINK)
	{
		...
		/* Get from 1st DMA that RTOS will use it */
		np = of_find_compatible_node(NULL, NULL, "ambarella,dma");
		/* To get scratpatch reg */
		reg = of_get_property(np, "reg", &len);
		if (reg && (len >= 0x10)) {
			vaddr = ioremap(be32_to_cpup(&reg[2]),
					be32_to_cpup(&reg[3]));
			/* For SPI-Nor boot, 1st and 2nd DMA channel must be used. */
			val = readl_relaxed(vaddr);
			switch (val & 0xFF) {
			case DMA_REQ_TX:
				/*DMA_ch0_control set to TX*/
				flash->dma_offset_tx = 0;
				/*DMA_ch1_control set to RX*/
				flash->dma_offset_rx = 0x10;
				break;
			default:
				/*DMA_ch1_control set to TX*/
				flash->dma_offset_tx = 0x10;
				/*DMA_ch0_control set to RX*/
				flash->dma_offset_rx = 0;
				break;
			}
		} else {
			dev_err(&pdev->dev, "Can not read DMA setting well!\n");
			errCode = -EINVAL;
			goto amb_spi_nor_probe_exit;
		}
	}
	...
	errCode = mtd_device_parse_register(&flash->mtd, NULL, NULL, NULL, 0);
	if (errCode < 0)
		goto amb_spi_nor_probe_free_command;
...
#ifdef CONFIG_PM
static int amb_spi_nor_suspend(struct platform_device *pdev, pm_message_t state)
{
	int errorCode = 0;
	struct amb_norflash *flash;
