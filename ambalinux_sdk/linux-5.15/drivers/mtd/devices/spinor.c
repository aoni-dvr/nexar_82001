...
int ambspi_read_reg(struct amb_norflash *flash, u32 datalen, u32 reg,
		    u8 * value)
{
	u32 tmp = 0;
	int i;
...
{
	flash->dmabuf =
	    dma_alloc_coherent(flash->dev, AMBA_SPINOR_DMA_BUFF_SIZE,
...
EXPORT_SYMBOL_GPL(ambspi_dma_config);
			...
			u32 dst, u32 len)
{
	...
	// DMA_chN_control
	writel_relaxed(0x1a800000,
		       flash->dmaregbase + flash->dma_offset_tx + 0x00);
	writel_relaxed(0x0, flash->dmaregbase + flash->dma_offset_tx + 0x0C);
	// DMA_chN_src_addr
	writel_relaxed(flash->dmaaddr + srcoffset,
		       flash->dmaregbase + flash->dma_offset_tx + 0x04);
	// DMA_chN_dest_addr
	writel_relaxed(flash->reg_phy_base + REG100,
		       flash->dmaregbase + flash->dma_offset_tx + 0x08);
	// DMA_chN_control
	writel_relaxed(0x9a800000 | len,
		       flash->dmaregbase + flash->dma_offset_tx + 0x00);

	tmp = readl_relaxed(flash->regbase + REG00);
	REGPREP(tmp, REG00_DATALEN_MASK, REG00_DATALEN_SHIFT, len);
	REGPREP(tmp, REG00_DUMMYLEN_MASK, REG00_DUMMYLEN_SHIFT, 0);
	REGPREP(tmp, REG00_ADDRLEN_MASK, REG00_ADDRLEN_SHIFT,
		flash->addr_width);
	REGPREP(tmp, REG00_CMDLEN_MASK, REG00_CMDLEN_SHIFT, 1);
	writel_relaxed(tmp, flash->regbase + REG00);
	...
	do {
		tmp = readl_relaxed(flash->regbase + REG2C);
		    ...
		    REGDUMP(tmp, REG2C_TXFIFOEMP_MASK, REG2C_TXFIFOEMP_SHIFT);
	} while (done != 0x0);
...
int ambspi_prog_data(struct amb_norflash *flash, u32 srcoffset, u32 dst,
		     u32 len)
{
	u32 tmp = 0;
	int ret = 0;
	...
	if (tail) {
		...
		tmp = 0;
		REGPREP(tmp, REG1C_TXFIFOLV_MASK, REG1C_TXFIFOLV_SHIFT,
			(256 - 32));
		writel_relaxed(tmp, flash->regbase + REG1C);
		...
		for (i = 0; i < tail; i++) {
			writeb_relaxed(*
				       (flash->dmabuf + srcoffset + bulk * 32 +
					i), flash->regbase + REG100);
		}
...
int ambspi_readdata_dma(struct amb_norflash *flash, u32 from, u32 len)
{
	u32 tmp = 0;

	// DMA_chN_control
	writel_relaxed(0x2a800000,
		       flash->dmaregbase + flash->dma_offset_rx + 0x00);
	writel_relaxed(0x0, flash->dmaregbase + flash->dma_offset_rx + 0x0C);
	// DMA_chN_src_addr
	writel_relaxed(flash->reg_phy_base + REG200,
		       flash->dmaregbase + flash->dma_offset_rx + 0x04);
	// DMA_chN_dest_addr
	writel_relaxed(flash->dmaaddr,
		       flash->dmaregbase + flash->dma_offset_rx + 0x08);
	// DMA_chN_control
	writel_relaxed(0xaa800000 | len,
		       flash->dmaregbase + flash->dma_offset_rx + 0x00);
...
int ambspi_read_data(struct amb_norflash *flash, u32 from, u32 len)
{
	...
	if (tail) {
		tmp = readl_relaxed(flash->regbase + REG00);
		REGPREP(tmp, REG00_DATALEN_MASK, REG00_DATALEN_SHIFT, tail);
		REGPREP(tmp, REG00_DUMMYLEN_MASK, REG00_DUMMYLEN_SHIFT,
			flash->dummy);
		REGPREP(tmp, REG00_ADDRLEN_MASK, REG00_ADDRLEN_SHIFT,
			flash->addr_width);
		REGPREP(tmp, REG00_CMDLEN_MASK, REG00_CMDLEN_SHIFT, 1);
		...
		if (flash->read_opcode == 0xbb) {
			REGPREP(tmp, REG04_DATALANE_MASK, REG04_DATALANE_SHIFT,
				1);
			REGPREP(tmp, REG04_ADDRLANE_MASK, REG04_ADDRLANE_SHIFT,
				1);
			REGPREP(tmp, REG04_RXLANE_MASK, REG04_RXLANE_SHIFT, 0);
		} else if (flash->read_opcode == 0xeb) {
			REGPREP(tmp, REG04_DATALANE_MASK, REG04_DATALANE_SHIFT,
...
int ambspi_init(struct amb_norflash *flash)
{
	...
	REGPREP(tmp, REG08_CHIPSEL_MASK, REG08_CHIPSEL_SHIFT,
		~(1 << SPINOR_DEV));
