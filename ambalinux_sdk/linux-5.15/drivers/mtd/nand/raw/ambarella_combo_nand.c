...
#ifdef CONFIG_ARCH_AMBARELLA_AMBALINK
	if (1 == ambalink_shm_layout.ambalink_version) {
		return 0;
	}
#endif
	if (host->is_spinand)
...
static const struct soc_device_attribute ambarella_nand_socinfo[] = {
	...
	{ .soc_id = "cv2fs", .data = &ambarella_nand_soc_data_v1 },
	{ .soc_id = "s6lm", .data = &ambarella_nand_soc_data_v1 },
	/* latest chips */
	{ .family = "Ambarella SoC", .data = &ambarella_nand_soc_data_v2 },
	{/* sentinel */}
};
...
#ifdef CONFIG_ARCH_AMBARELLA_AMBALINK
	/*dual os: do not switch between parallel nand and spinand*/
	host->pins = NULL;
	host->pins_nand = NULL;
#else
...
static int ambarella_nand_probe(struct platform_device *pdev)
{
	...
	host->dmabuf = dma_alloc_coherent(host->dev, AMBARELLA_NAND_BUFFER_SIZE,
...
err_exit0:
	dma_free_coherent(host->dev, AMBARELLA_NAND_BUFFER_SIZE,
...
static int ambarella_nand_remove(struct platform_device *pdev)
{
	...
	rval = mtd_device_unregister(nand_to_mtd(&host->chip));
	WARN_ON(rval);
	nand_cleanup(&host->chip);
