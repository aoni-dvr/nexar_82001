...
#ifdef CONFIG_ARCH_AMBARELLA_AMBALINK
static void amba_spinor_global_request(struct ambarella_spinor *flash)
{
	aipc_mutex_lock(AMBA_IPC_MUTEX_SPINOR);
	if (flash->nor_spi_irq >= 0)
		enable_irq(flash->nor_spi_irq);

	/* VLSI constrain: can not use the same ID at 2 DMAs */
	{
		int ret;

		ret = amba_spinor_setup_dma(flash);
		BUG_ON(ret != 0);
	}
}

static void amba_spinor_global_release(struct ambarella_spinor *flash)
{
	/* VLSI constrain: can not use the same ID at 2 DMAs */
	amba_spinor_release_dma(flash);
...
#ifdef CONFIG_ARCH_AMBARELLA_AMBALINK
	amba_spinor_global_request(amba_spinor);
#endif
...
#ifdef CONFIG_ARCH_AMBARELLA_AMBALINK
	if (ret < 0) {
		amba_spinor_global_release(amba_spinor);
		return ret;
	}
#else
	if (ret < 0)
...
#ifdef CONFIG_ARCH_AMBARELLA_AMBALINK
	// handler only handle datalenreach
	writel_relaxed(SPINOR_INTR_DATALENREACH,
