...
static int ambahb_mdio_poll_status(struct regmap *regscr, u32 regoff)
{
	int i;
	u32 value;

	for (i = 0; i < 1000; i++) {
		regmap_read(regscr, regoff, &value);
		if (!(value & 1))
			return 0;
		else
			mdelay(1);
...
static void ambarella_gmac_plat_exit(struct platform_device *pdev, void *priv)
{
/*
	struct amba_bsp_priv *bsp_priv = priv;
...
static int ambarella_gmac_plat_init(struct plat_stmmacenet_data *plat_dat)
{
	struct amba_bsp_priv *bsp_priv = plat_dat->bsp_priv;
	struct device *dev = &bsp_priv->pdev->dev;

	/* power-on-phy */
	if (gpio_is_valid(bsp_priv->pwr_gpio)) {
		dev_info(dev, "ambarella_gmac_powerup: output %u \n", !bsp_priv->pwr_gpio_active);
		gpio_direction_output(bsp_priv->pwr_gpio, !bsp_priv->pwr_gpio_active);
		msleep(20);
		dev_info(dev, "ambarella_gmac_powerup: output %u \n", bsp_priv->pwr_gpio_active);
		gpio_direction_output(bsp_priv->pwr_gpio, bsp_priv->pwr_gpio_active);
	...
	if (bsp_priv->ahb_mdio_clk_div) {
		plat_dat->mdio_read = &ambahb_mdio_read;
		plat_dat->mdio_write = &ambahb_mdio_write;
	}
	plat_dat->exit = ambarella_gmac_plat_exit;
...
static int ambarella_gmac_probe(struct platform_device *pdev)
{
	...
	ambarella_gmac_plat_init(plat_dat);
