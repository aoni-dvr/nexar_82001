...
static inline int ambarella_is_pcie_udma(struct dma_chan *chan,
					 struct device *filter_dev)
{
	dev_dbg(chan->device->dev, "dev_name(chan->device->dev) is %s\n",
		dev_name(chan->device->dev));
	dev_dbg(filter_dev, "dev_name(filter->dev) is %s\n",
		dev_name(filter_dev));
	if (strstr(dev_name(filter_dev), "pcie0"))
		return strstr(dev_name(chan->device->dev), "udma") &&
		       strstr(dev_name(chan->device->dev), "pcie0");
	else if (strstr(dev_name(filter_dev), "pcie1"))
		return strstr(dev_name(chan->device->dev), "udma") &&
		       strstr(dev_name(chan->device->dev), "pcie1");
	else if (strstr(dev_name(filter_dev), "pcie2"))
		return strstr(dev_name(chan->device->dev), "udma") &&
		       strstr(dev_name(chan->device->dev), "pcie2");
	else
		return false;
}
...
static bool epf_dma_filter_fn(struct dma_chan *chan, void *node)
{
	...
	if (IS_ENABLED(CONFIG_ARCH_AMBARELLA) &&
	    ambarella_is_pcie_udma(chan, filter->dev) &&
