...
static struct mutex amb_pinctrl_mutex[MAX_BANK_NUM];
static char param_amb_pctrl_mutex[MAX_BANK_NUM][2];
#endif
			...
			unsigned int selector, unsigned int group)
{
	...
	for (i = 0; i < grp->num_pins; i++) {
		...
		mutex_lock(&amb_pinctrl_mutex[bank]);
		spin_lock_irqsave(&soc->lock, flags);

		amb_pinmux_set_altfunc(soc, bank, offset,  grp->alt[i]);

		spin_unlock_irqrestore(&soc->lock, flags);
		mutex_unlock(&amb_pinctrl_mutex[bank]);
	}

#else // no CONFIG_ARCH_AMBA_CAMERA
#ifdef CONFIG_ARCH_AMBARELLA_AMBALINK
	(void)soc;
	...
	for (i = 0; i < num_configs; i++) {
		...
		if (CFG_PULL_PRESENT(config)) {
			regmap_update_bits(soc->pull_regmap,
				PULL_OFFSET(soc->pull_dir, bank), 0x1 << offset,
				CONF_TO_PULL_VAL(config) << offset);

			regmap_update_bits(soc->pull_regmap,
				PULL_OFFSET(soc->pull_en, bank), 0x1 << offset,
		...
		if (CFG_DS_PRESENT(config)) {
			...
			regmap_update_bits(soc->ds_regmap,
				DS0_OFFSET(bank), 0x1 << offset,
				((CONF_TO_DS_VAL(config) >> 1) & 0x1) << offset);
			regmap_update_bits(soc->ds_regmap,
				DS1_OFFSET(bank), 0x1 << offset,
			...
			struct seq_file *s, unsigned int pin)
{
	...
	regmap_read(soc->pull_regmap, PULL_OFFSET(soc->pull_en, bank), &pull_en);
	pull_en = (pull_en >> offset) & 0x1;
	regmap_read(soc->pull_regmap, PULL_OFFSET(soc->pull_dir, bank), &pull_dir);
	pull_dir = (pull_dir >> offset) & 0x1;
	seq_printf(s, " pull: %s, ", pull_en ? pull_dir ? "up" : "down" : "disable");

	regmap_read(soc->ds_regmap, DS0_OFFSET(bank), &ds0);
	ds0 = (ds0 >> offset) & 0x1;
	regmap_read(soc->ds_regmap, DS1_OFFSET(bank), &ds1);
	ds1 = (ds1 >> offset) & 0x1;
...
#ifdef CONFIG_ARCH_AMBA_CAMERA
static int amb_pctrl_mutex(const char *val, const struct kernel_param *kp)
{
	char *data = strstrip((char *)val);
	unsigned int group = data[0] - '0';

	if (data[1] == '+') {
		mutex_lock(&amb_pinctrl_mutex[group]);
	} else if (data[1] == '-') {
		mutex_unlock(&amb_pinctrl_mutex[group]);
	} else {
        printk(KERN_ERR "Error character:%c\n\n", data[1]);
        printk(KERN_ERR "[Hint] Write N+ for locking group N; Write N- for unlocking group N.\n");
	}

	return param_set_charp(data, kp);
}

static struct kernel_param_ops param_ops_amb_pctrl_mutex = {
	.get = param_get_charp,
	.set = amb_pctrl_mutex,
	.free = param_free_charp,
};
module_param_cb(param_amb_pctrl_mutex, &param_ops_amb_pctrl_mutex, &(param_amb_pctrl_mutex), 0644);
#endif

#if defined(CONFIG_ATF_SUSPEND_SRAM)
static irqreturn_t gpio_irq_handler(int irqno, void *dev)
{
    return IRQ_HANDLED;
}
#endif
...
static int amb_pinctrl_probe(struct platform_device *pdev)
{
	...
	int i, rval;
#if defined(CONFIG_ATF_SUSPEND_SRAM)
	uint32_t wakeup_gpio = 0xFFFFFFFF;
#endif
	...
	if (of_property_read_u32_index(np, "amb,pull-regmap", 1, &soc->pull_en))
		return dev_err_probe(&pdev->dev, -EINVAL,
							 "couldn't get pull_en offset");

	if (of_property_read_u32_index(np, "amb,pull-regmap", 2, &soc->pull_dir))
		return dev_err_probe(&pdev->dev, -EINVAL,
							 "couldn't get pull_dir offset");
	...
	for (i = 0; i < MAX_BANK_NUM; i++)
		mutex_init(&amb_pinctrl_mutex[i]);
#endif
...
#if defined(CONFIG_ATF_SUSPEND_SRAM)
    if (of_property_read_u32(np, "amb,wake-up", &wakeup_gpio) == 0) {
        int irqno = gpio_to_irq(wakeup_gpio);

        rval = request_irq(irqno, gpio_irq_handler, IRQF_TRIGGER_RISING, "gpio-wake", NULL);
        if (rval) {
            return dev_err_probe(&pdev->dev, rval, "gpio irq request failed!");
        }
        enable_irq_wake(irqno);
    }
    suspend_set_wakeup(wakeup_gpio);
#endif
...
static int amb_pinctrl_suspend(void)
{
	...
	for (i = 0; i < soc->bank_num; i++) {
		reg_offset = PULL_OFFSET(soc->pull_en, i);
		regmap_read(soc->pull_regmap, reg_offset, &soc->pm[i].pull[0]);
#if defined(CONFIG_ATF_SUSPEND_SRAM)
		regmap_write(soc->pull_regmap, reg_offset, 0); // disable internal pull-up/down
#endif
		reg_offset = PULL_OFFSET(soc->pull_dir, i);
		regmap_read(soc->pull_regmap, reg_offset, &soc->pm[i].pull[1]);

		reg_offset = DS0_OFFSET(i);
		regmap_read(soc->ds_regmap, reg_offset, &soc->pm[i].ds[0]);
		reg_offset = DS1_OFFSET(i);
		regmap_read(soc->ds_regmap, reg_offset, &soc->pm[i].ds[1]);
...
static void amb_pinctrl_resume(void)
{
	...
	for (i = 0; i < soc->bank_num; i++) {
		reg_offset = PULL_OFFSET(soc->pull_en, i);
		regmap_write(soc->pull_regmap, reg_offset, soc->pm[i].pull[0]);
		reg_offset = PULL_OFFSET(soc->pull_dir, i);
		regmap_write(soc->pull_regmap, reg_offset, soc->pm[i].pull[1]);

		reg_offset = DS0_OFFSET(i);
		regmap_write(soc->ds_regmap, reg_offset, soc->pm[i].ds[0]);
		reg_offset = DS1_OFFSET(i);
		regmap_write(soc->ds_regmap, reg_offset, soc->pm[i].ds[1]);
