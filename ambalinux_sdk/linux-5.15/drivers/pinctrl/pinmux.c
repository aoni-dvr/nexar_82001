...
static unsigned long gpio_reclaim = 0x0;
module_param(gpio_reclaim,ulong, 0644);
MODULE_PARM_DESC(gpio_reclaim, "allow gpio reclaim");
#endif
		       ...
		       struct pinctrl_gpio_range *gpio_range)
{
	...
	if(gpio_reclaim){
		...
		if ((!gpio_range || ops->strict) &&
			...
			pr_debug("hw->hw %s re-claim %s -> %s\n", desc->name, desc->mux_owner, owner);
			pinctrl_gpio_free(pin);
			pr_debug("%s(%d) pinctrl_gpio_free(%d)\n", __func__, __LINE__, pin);
			pr_debug("%s(%d) desc->mux_usecount %d\n", __func__, __LINE__, desc->mux_usecount);
			if (desc->mux_usecount > 0) {
				desc->mux_usecount--;
			}
		...
		/*hw -> sw*/
		else if (gpio_range && (desc->mux_owner != NULL) && (desc->gpio_owner == NULL)) {
			pr_debug("hw -> sw %s re-claim %s -> %s\n", desc->name, desc->mux_owner, owner);
			pinmux_free_gpio(pctldev, pin, gpio_range);
			pr_debug("%s(%d) amb_pinmux_free(%d)\n", __func__, __LINE__, pin);
			pr_debug("%s(%d) desc->mux_usecount %d\n", __func__, __LINE__, desc->mux_usecount);
		}
		/*sw -> hw*/
		else if (!gpio_range && (desc->mux_owner == NULL) && (desc->gpio_owner != NULL)) {
			pr_debug("sw -> hw %s re-claim %s -> %s\n", desc->name, desc->gpio_owner, owner);
			pinctrl_gpio_free(pin);
			pr_debug("%s(%d) pinctrl_gpio_free(%d)\n", __func__, __LINE__, pin);
			pr_debug("%s(%d) desc->mux_usecount %d\n", __func__, __LINE__, desc->mux_usecount);
			if (desc->mux_usecount > 0) {
