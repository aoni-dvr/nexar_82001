...
#if defined(CONFIG_AMBARELLA_RPROC)
#include <linux/remoteproc.h>
#include <soc/ambarella/ambalink/remoteproc.h>
#include <soc/ambarella/ambalink/remoteproc_cfg.h>
#endif
...
#ifdef CONFIG_RPMSG_AMBA_CLNT
static phys_addr_t ambarella_get_virtio_base(struct virtio_device *vdev)
{
...
static void *get_a_tx_buf_slv(struct virtproc_info *vrp, u16 *idx, u32 *len)
{
	void *ret;
	...
	ret = virtqueue_get_buf_slv(vrp->svq, idx, len);
	if (ret) {
		/* msg for slave if physical address. */
		phys_addr_t ambarella_get_virtio_base(struct virtio_device *vdev);

		phys_addr_t addr = ambarella_get_virtio_base(vrp->vdev);

		BUG_ON((phys_addr_t)ret < addr);
		addr = (phys_addr_t)ret - addr;
		ret  = (struct rpmsg_hdr *)(addr + vrp->sbufs);
	}
...
static void *get_a_rx_buf_slv(struct virtproc_info *vrp, u16 *idx, u32 *len)
{
	void *ret;
	...
	ret = virtqueue_get_buf_slv(vrp->rvq, idx, len);
	if (ret) {
		/* msg for slave if physical address. */
		phys_addr_t ambarella_get_virtio_base(struct virtio_device *vdev);
...
#endif // CONFIG_RPMSG_AMBA_CLNT

/* super simple buffer "allocator" that is just enough for now */
#if defined(CONFIG_AMBARELLA_RPROC) && !defined(CONFIG_RPMSG_AMBA_CLNT)
				     ...
				     void *data, int len, bool wait)
{
...
#if defined(CONFIG_AMBARELLA_RPROC)
		/* it is normal when veth0, veth1, veth2 not ifup at the same time */
		dev_dbg(dev, "invalid addr (src 0x%x, dst 0x%x)\n", src, dst);
#else
...
#elif defined(CONFIG_RPMSG_AMBA_CLNT)
	if (virtio_has_feature(vrp->vdev, VIRTIO_RPMSG_F_SLV)) {
		msg = get_a_tx_buf_slv(vrp, &idx, &buf_len);
	} else {
	...
	while (!msg) {
		 ...
		 */
#if defined(CONFIG_RPMSG_AMBA_CLNT)
		if (virtio_has_feature(vrp->vdev, VIRTIO_RPMSG_F_SLV)) {
...
#ifdef CONFIG_RPMSG_AMBA_CLNT
	if (virtio_has_feature(vrp->vdev, VIRTIO_RPMSG_F_SLV)) {
		err = virtqueue_add_buf_slv(vrp->svq, idx, buf_len);
	} else {
...
static void rpmsg_recv_done(struct virtqueue *rvq)
{
...
#elif defined(CONFIG_RPMSG_AMBA_CLNT)
	if (virtio_has_feature(vrp->vdev, VIRTIO_RPMSG_F_SLV)) {
		msg = get_a_rx_buf_slv(vrp, &idx, &len);
	} else {
...
#ifdef CONFIG_RPMSG_AMBA_CLNT
		if (virtio_has_feature(vrp->vdev, VIRTIO_RPMSG_F_SLV))
			err = virtqueue_add_buf_slv(vrp->rvq, idx, len);
		if (err)
			break;
#endif
#ifdef RPMSG_DEBUG
	...
	// TODO: if rvq_rx_ack == svq_tx_kick then not kick
#ifndef CONFIG_RPMSG_AMBA_CLNT
...
#ifdef CONFIG_RPMSG_AMBA_CLNT
static int ambarella_get_rpmsg_size(struct virtio_device *vdev)
{
	struct rproc_vdev *rvdev = vdev_to_rvdev(vdev);
	struct rproc *rproc = rvdev->rproc;
	struct ambarella_rproc_pdata *pdata = rproc->priv;
...
static int ambarella_get_rpmsg_num(struct virtio_device *vdev)
{
	struct rproc_vdev *rvdev = vdev_to_rvdev(vdev);
	struct rproc *rproc = rvdev->rproc;
	struct ambarella_rproc_pdata *pdata = rproc->priv;
...
struct rpmsg_device * rpmsg_create_new_device(struct virtio_device *vdev, const char *name)
{
	...
	memset(chinfo.name, 0, RPMSG_NAME_SIZE);
	len = strlen(name);
	if (len > (RPMSG_NAME_SIZE - 2))
		len = RPMSG_NAME_SIZE - 2;
	strncpy(chinfo.name, name, len);
	chinfo.src = addr;
	chinfo.dst = RPMSG_ADDR_ANY;

	rpdev = __rpmsg_create_channel(vrp, &chinfo);
	if (rpdev)
		addr++;
	else
		return NULL;
...
EXPORT_SYMBOL(rpmsg_create_new_device);
#endif // CONFIG_RPMSG_AMBA_CLNT
...
#if defined(CONFIG_RPMSG_AMBA_CLNT)
	vrp->num_bufs = ambarella_get_rpmsg_num(vdev);
#else
...
#elif defined(CONFIG_RPMSG_AMBA_CLNT)
	vrp->buf_size = ambarella_get_rpmsg_size(vdev);
#else
...
#ifdef CONFIG_RPMSG_AMBA_CLNT
	if (virtio_has_feature(vdev, VIRTIO_RPMSG_F_SLV)) {
		// Slave/Client, swap
		vrp->sbufs = bufs_va;
		...
		while (1) {
			...
			/* waiting synchronous with the master */
			if (vdev->config->get_status(vdev) & VIRTIO_CONFIG_S_MASTER_OK) {
				break;
			}
...
static unsigned int features[] = {
	VIRTIO_RPMSG_F_NS,
#if defined(CONFIG_AMBARELLA_RPROC)
	VIRTIO_RPMSG_F_SLV,	/* 0: Master, 1: Slave/Remote */
#endif
};
