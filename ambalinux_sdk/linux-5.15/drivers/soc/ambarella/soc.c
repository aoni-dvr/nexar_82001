...
#if defined(CONFIG_AMBARELLA_RPROC) || defined(CONFIG_AMBARELLA_RPROC_MODULE)
#include <soc/ambarella/ambalink/ambalink_cfg.h>

struct ambalink_shared_memory_layout ambalink_shm_layout;
EXPORT_SYMBOL(ambalink_shm_layout);
...
static struct proc_dir_entry *ambroot_proc_dir;

struct proc_dir_entry *ambarella_proc_dir(void)
{
	return ambroot_proc_dir;
}
EXPORT_SYMBOL(ambarella_proc_dir);
...
static int __init ambarella_soc_init(void)
{
	...
	ambroot_proc_dir = proc_mkdir("ambarella", NULL);
	if (IS_ERR_OR_NULL(ambroot_proc_dir)) {
...
arch_initcall(ambarella_init_misc);
#else
