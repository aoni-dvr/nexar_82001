			...
			bool dma_to_memory)
{
	...
	if (dma_to_memory) {
		...
		dma_buf = kmalloc(AMBA_UART_RX_DMA_BUFFER_SIZE, GFP_KERNEL | GFP_DMA);
		...
		dma_phys = dma_map_single(amb_port->port.dev,
			...
			DMA_TO_DEVICE);
		if(dma_mapping_error(amb_port->port.dev, dma_phys)) {
			dev_err(amb_port->port.dev,
...
#if defined(CONFIG_ATF_SUSPEND_SRAM)
	if (of_find_property(pdev->dev.of_node, "amb,wake-used", NULL)) {
	...
	pm_message_t state)
{
	...
	if (device_can_wakeup(&pdev->dev)) {
		if (amb_port->txdma_used || amb_port->rxdma_used) {
			// Reset setting to no dma case.
			writel_relaxed(0, amb_port->port.membase + UART_DMAE_OFFSET);

			writel_relaxed(DEFAULT_AMBARELLA_UART_IER | UART_IE_PTIME,
				amb_port->port.membase + UART_IE_OFFSET);
			writel_relaxed(UART_FC_FIFOE | UART_FC_RX_2_TO_FULL | UART_FC_TX_EMPTY |
				UART_FC_XMITR | UART_FC_RCVRR, amb_port->port.membase + UART_FC_OFFSET);
			writel_relaxed(DEFAULT_AMBARELLA_UART_IER, amb_port->port.membase + UART_IE_OFFSET);
		}
	}
#endif
