...
void ambafs_update_inode(struct inode *inode, struct ambafs_stat *stat)
{
	...
	AMBAFS_DMSG("%s: 1 inode->i_size = %lld,  stat->size = %lld, inode->i_mtime.tv_sec = %lld, stat->mtime = %lld, inode->i_opflags = 0x%x\r\n",
	...
	AMBAFS_DMSG("%s: 2 inode->i_size = %lld,  stat->size = %lld, inode->i_mtime.tv_sec = %lld, stat->mtime = %lld\r\n",
	...
	inode->i_ctime.tv_sec = stat->ctime;
