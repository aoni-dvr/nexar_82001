...
#define AMBAFS_MSG_MIN_LEN (sizeof(struct ambafs_msg) + sizeof(struct ambafs_io)+  sizeof(struct ambafs_bh))
...
static void readpage_cb(void *priv, struct ambafs_msg *msg, int len)
{
	...
	for (page_idx = 0; page_idx < nr_pages; page_idx++) {
		struct page *page;
		//void *virt = phys_to_virt((phys_addr_t)io->bh[page_idx].addr);
...
{
	char buf[AMBAFS_MSG_MIN_LEN], msg_len;
...
{
	int *buf = kmalloc(RPMSG_BUF_SIZE, GFP_KERNEL);
	int io_pages = 0, err, msg_len = 0;
	struct ambafs_msg *msg = (struct ambafs_msg*) buf;
	struct page *page;
	loff_t offset = 0;
			...
			struct page *page, void *fsdata)
{
	...
	ram_aops.write_end(file, mapping, pos, len, copied, page, fsdata);
