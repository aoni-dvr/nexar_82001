 ...
 */
static int ambafs_create(struct user_namespace *mnt_userns, struct inode *inode, struct dentry *dentry, umode_t mode,
				bool excl)
{
	int buf[128];
	struct ambafs_msg  *msg  = (struct ambafs_msg *)buf;
 ...
 */
static int ambafs_mkdir(struct user_namespace *mnt_userns, struct inode *inode,
 ...
 */
static int ambafs_rename(struct user_namespace *mnt_userns, struct inode *old_dir, struct dentry *old_dentry,
...
int ambafs_get_qstat(struct dentry *dentry, void *buf, int size)
{
	...
	AMBAFS_DMSG("%s: path = %s, quick_stat result phy address = 0x%llx \r\n", __func__, path, msg->parameter[0]);
...
static int ambafs_d_revalidate(struct dentry *dentry, unsigned int flags)
{
	...
	if (inode && is_bad_inode(inode))
		goto invalid;
	else if (time_before64((__u64)dentry->d_time, get_jiffies_64()) ||
		 (flags & LOOKUP_REVAL)) {
		...
		if (type != AMBAFS_STAT_DIR) {
			AMBAFS_DMSG("%s fail: dentry = %px name = %s type = %d \r\n", __func__,dentry,dentry->d_name.name, type);
			goto out;
		} else {
			ts.tv_sec = 0;
 ...
 */
static int ambafs_getattr(struct user_namespace *mnt_userns, const struct path *path, struct kstat *stat,
...
int ambafs_remote_set_timestamp(struct dentry *dentry, struct iattr *attr)
{
	...
	time64_to_tm(attr->ia_atime.tv_sec, 0, &time);
	fill_time_info(&stat->atime, &time);
	time64_to_tm(attr->ia_mtime.tv_sec, 0, &time);
	fill_time_info(&stat->mtime, &time);
	time64_to_tm(attr->ia_ctime.tv_sec, 0, &time);
...
int ambafs_remote_set_size(struct dentry *dentry, u64 size)
{
	...
	AMBAFS_DMSG("%s file %s set size %lld\n", __func__, path, size);
...
int ambafs_setattr(struct user_namespace *mnt_userns, struct dentry *dentry, struct iattr *attr)
{
