...
/*
 * ---------------------------- //CV5 SRAM 0x20E0030000
 * |                          |
 * |      SR_ENTER_CODE       | code size
 * |                          |
 * ----------------------------
 * |                          |
 * |      SR_EXIT_CODE        | code size
 * |                          |
 * ----------------------------
 * |                          |
 * |      IDLE2WAKE_CODE      | code size
 * |                          |
 * ----------------------------
 * |                          |
 * |      IDLE2WAKE_STACK     | stack size
 * |                          |
 * ----------------------------
 * |                          |
 * |      IDLECORE_CODE       | code size
 * |                          | code size - sizeof(uint64_t)*2 = wakecore_entry
 * ---------------------------- code size - sizeof(uint64_t) = wakecore_status
 * |                          |
 * |      IDLECORE_STACK      | stack size
 * |                          |
 * ----------------------------
 * ---------------------------- uint32 wakeup gpio pin
 */
#define STACK_SIZE              (0x400)
#define CODE_SIZE               (0x400)
struct platform_fast_resume_code {
    unsigned char ddr_sr_enter[CODE_SIZE];
    unsigned char ddr_sr_exit[CODE_SIZE];
    unsigned char idle2wake_code[CODE_SIZE];
    unsigned char idle2wake_stack[STACK_SIZE];
    unsigned char idlecore_code[CODE_SIZE - sizeof(uint64_t)*2];
    uint64_t wakecore_entry;
    uint64_t wakecore_status;
    unsigned char idlecore_stack[STACK_SIZE];
    uint32_t wakeup_gpio;
};

struct platform_fast_resume_info {
    bool enabled;
    uintptr_t code_base;
    uint32_t stack_size;
    uint32_t code_size;
...
extern void suspend_set_addr(const uintptr_t base, uint32_t code_size, uint32_t stack_size);
extern void suspend_set_wakeup(uint32_t gpio);
extern bool suspend_resume_supported(void);
#endif
