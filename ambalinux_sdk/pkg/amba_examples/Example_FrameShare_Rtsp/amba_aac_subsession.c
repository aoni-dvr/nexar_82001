...
#define RTP_FUA_HDR_LEN     (RTP_HEADER_SIZE+1)          //(RTP_HEADER_SIZE + 1)  add 1 for FUA
#define RTP_AAC_HDR_LEN     (4)          //AAC HEADER
                    ...
                    unsigned len)
{
    ...
    // (and only) "AU Header":
    last_start = buf + RTP_FUA_HDR_LEN;
    last_start[0] = 0; last_start[1] = 16 /* bits */; // AU-headers-length
    last_start[2] = len >> 5; last_start[3] = (len&0x1F)<<3;
    ...
    if(len > DEFAULT_MTU) {
        ...
        priv->rtp_payload_info.len = len + RTP_HEADER_SIZE + RTP_AAC_HDR_LEN;
        ...
        if(cur_frame_info->addr_info.len > 0) {
            if(priv->en_rtcp){
                if(priv->total_read_frames==0){
                    stream_restart=1;
                } else if((priv->total_read_frames>=min_sr_period)||(stream_restart)) {
            ...
            if(stream_restart) {
                unsigned int tmp_ts=priv->cur_timestamp;
...
static void read_avail_pb_cb(struct ev_loop* loop, ev_io* w, int revent) {
    ...
    if (priv->total_read_pb_frames != 0) { //not 1st frame
        ...
        if((ts_diff < 0) || (ts_diff < TicksPerFrame*4/5)){ //not yet.
            usleep( (TicksPerFrame-ts_diff)/(sampling_frequency/1000) * 100);
            return;
        }
    }
    ...
    if(what == 'd') {
        ...
        //fprintf(stderr, "afno=%u, apts=%lu\n",cur_frame_info->frame_num,cur_frame_info->pts);
        if(cur_frame_info->addr_info.len > 0) {
            if(priv->en_rtcp){
                if(priv->total_read_pb_frames==0){
                    stream_restart = 1;
                } else if((priv->total_read_pb_frames>=min_sr_period)||(stream_restart)) {
                    priv->rtp->need_to_send_sr_sdes = 1;
...
static char *get_ConfigStr(struct audio_frame_reader* thiz)
{
    char *fConfigStr = NULL;
    const unsigned char fixedHeader[4]={0xff,0xf1,0x4c,0x80}; // it's actually 3.5 bytes long. Fix data for Amba AAC
    const unsigned char profile = (fixedHeader[2]&0xC0)>>6; // 2 bits
    ...
    if(thiz->get_audio_config(thiz, &format, &samplerate, &channel)<0){
    ...
    for(sampling_frequency_index=0; sampling_frequency_index<16; sampling_frequency_index++){
        if(samplingFrequencyTable[sampling_frequency_index]==(unsigned)samplerate){
            ...
            //fuSecsPerFrame = (1024/*samples-per-frame*/ * 1000000) / samplerate/*samples-per-second*/;
            //printf("TimeStamp diff=%u\n",fuSecsPerFrame);
            TicksPerFrame = 1024;//90000*1024/samplerate;
            min_sr_period = (samplerate * 5)/TicksPerFrame;
            break;
        }
    }
    ...
    fConfigStr = (char *)malloc(16);
    if(fConfigStr!=NULL) {
        sprintf(fConfigStr, "%02X%02x", audioSpecificConfig[0], audioSpecificConfig[1]);
    } else {
        printf("Cannot create fConfigStr!!\n");
    }
...
static const char* get_aux_sdp(struct media_subsession_s* thiz)
{
    ...
    char const* fmtMode = "AAC-hbr";
    char const* EmptyConfString = "\"\"";
    char *fConfigString = NULL;
    int fConfigString_needfree = 1;
    char const* fmtpFmt =
            "a=fmtp:%d "
    ...
    fConfigString = get_ConfigStr((audio_frame_reader_t*) priv->frame_reader);
    if(fConfigString == NULL){
        fConfigString_needfree = 0;
        fConfigString = (char *)EmptyConfString;
    }
...
static const char* get_sdp_lines(struct media_subsession_s* thiz)
{
    ...
    if(1){
        ...
        sprintf(priv->sdp_lines, sdpFmt,
            ...
            rtpPayloadType, sampling_frequency, channel_configuration,// a=rtpmap:... (if present)
 ...
 * public function
 ********************************************************************************/
void amba_aac_subsession_release(media_subsession_t* thiz);
media_subsession_t* amba_aac_subsession_create(rtsp_server_t* srv, void* streamer,
        unsigned int en_rtcp, unsigned int playback)
{
    ...
    if (playback ==0) {
        rval = media_subsession_init(media_sub, srv, en_rtcp,
                ...
                1, // use dsp clock
                6974,
                RTP_FUA_HDR_LEN + RTP_AAC_HDR_LEN,
                streamer,
                read_avail_cb);
    } else {
        rval = media_subsession_init(media_sub, srv, en_rtcp,
    ...
    media_sub->release = amba_aac_subsession_release;
