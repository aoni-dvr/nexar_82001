...
uint32_t g_seq_num[CHNUM];
uint32_t g_frame_num[CHNUM];
uint32_t g_channel[MAX_FLOW_AMOUNT];
...
static uint32_t RefCV_TransferSourceData(uint32_t Channel, memio_source_recv_picinfo_t *pInfo)
{
    ...
    if (Rval != FLEXIDAGIO_OK) {
        ...
        g_seq_num[Channel]++;
        g_frame_num[Channel] = pInfo->pic_info.frame_num;
#if DEBUG
...
static uint32_t RefCV_TransferSinkData(uint32_t Channel, uint32_t type, uint32_t BufOutAddr, uint32_t BufOutSize)
{
    ...
    SinkOut.type = type;
    SinkOut.cvtask_frameset_id = g_frame_num[Channel];
    SinkOut.num_of_io = g_seq_num[Channel];
...
{
    uint32_t ret = 0;
    Preference_GetSrcChan(id, &g_channel[id]);

    RefCV_TransferInit(g_channel[id], TRANS_OUTPUT_TYPE);
    return ret;
}
...
uint32_t CamUtil_Release(uint32_t id)
{
    ...
    RefCV_TransferDeinit(g_channel[id]);
    return ret;
}
...
uint32_t CamUtil_GetFrame(uint32_t id, memio_source_recv_picinfo_t *pInfo)
{
    ...
    ret = GetFrame(g_channel[id], pInfo);
    return ret;
}
...
uint32_t CamUtil_SendOSD(uint32_t id, memio_sink_send_out_t *out)
{
    ...
    ret = SendResult(g_channel[id], out);
    return ret;
}
