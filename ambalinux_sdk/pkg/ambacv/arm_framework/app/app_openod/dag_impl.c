        ...
        *result = NULL;
        ...
        //config NMS
        {
            ...
            if (roi_cfg.roi_start_col == 9999U) {
                Conf.roi_start_col = (Conf.roi_w - NET_INPUT_W) / 2U;
            } else if (roi_cfg.roi_start_col > (Conf.roi_w - NET_INPUT_W)) {
                Conf.roi_start_col = Conf.roi_w - NET_INPUT_W;
            } else {
                Conf.roi_start_col = roi_cfg.roi_start_col;
            }
            //my_ctx.roi_start_row  = (my_ctx.roi_h - NET_INPUT_H) / 2U;
            if (roi_cfg.roi_start_row == 9999U) {
                Conf.roi_start_row = (Conf.roi_h - NET_INPUT_H) / 2U;
            } else if (roi_cfg.roi_start_row > (Conf.roi_h - NET_INPUT_H)) {
                Conf.roi_start_row = Conf.roi_h - NET_INPUT_H;
            } else {
                Conf.roi_start_row = roi_cfg.roi_start_row;
            }
            Conf.net_in_w = NET_INPUT_W;
            Conf.net_in_h = NET_INPUT_H;
        ...
        if (ret != 0) {
            ...
            *result = (void *)&od_results[id];
            ...
            if (debug) {
                printf("[Algo_0_Process] done! fn[%u] num_objects=%u\n", in->pic_info.frame_num, box_ammount);
            }
        }
    }
