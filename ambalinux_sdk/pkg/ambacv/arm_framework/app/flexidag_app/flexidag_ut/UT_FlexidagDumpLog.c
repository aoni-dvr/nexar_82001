...
uint32_t main(void)
{
    ...
    const char log_path[128] = "/tmp/SD0/flexidag_log.txt";
    uint32_t Rval = 0U;
    ...
    if (Rval == 0U) {
        ...
        if (Rval == 0U) {
            Rval = AmbaCV_FlexidagDumpLog(&handle, log_path, FLEXILOG_CORE0 | FLEXILOG_VIS_CVTASK);
        }
        Rval |= Do_FlexidagClose(&handle, &mem);
    }

    if (Rval != 0U) {
        printf("UT_FlexidagDumpLog fail (0x%x)\n", Rval);
    } else {
        printf("UT_FlexidagDumpLog success\n", Rval);
    }
