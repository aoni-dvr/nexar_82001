...
static void RunCallback(void *vpHandle, uint32_t flexidag_output_num, flexidag_memblk_t *pblk_Output, void *vpParameter)
{
    static uint32_t ret = 0U;

    printf("RunCallback() called: %d\n", ret);
    ret++;
}
...
uint32_t main(void)
{
    ...
    if (Rval == 0U) {
        ...
        if (Rval == 0U) {
            ...
            if (Rval == 0U) {
                for(i = 0; i < 5; i++) {
                    Rval = AmbaCV_FlexidagRunNonBlock(&handle, RunCallback, cb_param, &mem.in_buf, &mem.out_buf, &token_id);
                    if (Rval == 0U) {
                        Rval = AmbaCV_FlexidagWaitRunFinish(&handle, token_id, &run_info);
                        if (Rval == 0U) {
    ...
    if (Rval != 0U) {
        printf("UT_FlexidagRunNonBlock fail (0x%x)\n", Rval);
    } else {
        printf("UT_FlexidagRunNonBlock success\n", Rval);
    }
