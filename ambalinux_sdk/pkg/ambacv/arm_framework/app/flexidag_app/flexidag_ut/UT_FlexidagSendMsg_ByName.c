...
static void Do_Prepare_Flowtable_Name(AMBA_CV_FLEXIDAG_NAME_s *flowtable_name)
{
    static char cvtask_name[64] = BISENET_CVTASK;

    flowtable_name->instance = NULL;
    flowtable_name->cvtask = cvtask_name;
    flowtable_name->algorithm = NULL;
    flowtable_name->step = NULL;
}

static void Do_Prepare_Msg(AMBA_CV_FLEXIDAG_MSG_s *msg, uint16_t flow_id, uint32_t *loop)
{
    msg->flow_id = flow_id;
    msg->vpMessage = loop;
...
static uint32_t Do_FlexidagRun(AMBA_CV_FLEXIDAG_HANDLE_s *handle, flexidag_memblk_set_s *mem, uint32_t loop)
{
    ...
    if (Rval == 0U) {
        Rval = Do_Show_Output(loop, &mem->out_buf);
    }
...
uint32_t main(void)
{
    ...
    if (Rval == 0U) {
        ...
        if (Rval == 0U) {
            ...
            if (Rval == 0U) {
                Do_Prepare_Flowtable_Name(&flowtable_name);
                Rval = AmbaCV_FlexidagGetFlowIdByName(&handle, flowtable_name, 4, flow_id_array, &num_found);
                for(i = 0; i < 5; i++) {
                    Do_Prepare_Msg(&msg, flow_id_array[0], &i);
                    Rval = AmbaCV_FlexidagSendMsg(&handle, &msg);
                    if (Rval == 0U) {
                        Rval = Do_FlexidagRun(&handle, &mem, i);
                    }
                    if (Rval != 0U) {
                        break;
                    }
    ...
    if (Rval != 0U) {
        printf("UT_FlexidagSendMsg_ByName fail (0x%x)\n", Rval);
    } else {
        printf("UT_FlexidagSendMsg_ByName success\n", Rval);
    }
