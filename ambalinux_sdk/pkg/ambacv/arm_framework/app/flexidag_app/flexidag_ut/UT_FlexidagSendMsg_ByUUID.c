...
uint32_t main(void)
{
    ...
    if (Rval == 0U) {
        ...
        if (Rval == 0U) {
            ...
            if (Rval == 0U) {
                Rval = AmbaCV_FlexidagGetFlowIdByUUID(&handle, BISENET_CVTASK_UUID, &flow_id);
                if (Rval == 0U) {
                    for(i = 0; i < 5; i++) {
                        Do_Prepare_Msg(&msg, flow_id, &i);
    ...
    if (Rval != 0U) {
        printf("UT_FlexidagSendMsg_ByUUID fail (0x%x)\n", Rval);
    } else {
        printf("UT_FlexidagSendMsg_ByUUID success\n", Rval);
    }
