...
#ifndef VISORC_CV22FS_I_FILE
#define VISORC_CV22FS_I_FILE
...
uint32_t krn_visorc_init(void)
{
    ...
    krn_module_printU5("[AMBACV] TARGET CHIP   :   CV22FS \n",0U,0U,0U,0U,0U);
    return retcode;
}
...
uint32_t krn_visorc_finalize_loading(const ambacv_mem_t* arg)
{
    ...
    if(arg == NULL) {
        ...
        if(arg->vp.base != 0U) {
            retcode = krn_cache_clean(krn_c2v((uint32_t)arg->vp.base), (uint32_t)arg->vp.size);
            krn_module_printU5("\tVP:   [0x%08x ++ 0x%08x]\n",
                               krn_c2p((uint32_t)arg->vp.base), (uint32_t)arg->vp.size, 0U, 0U, 0U);
        } else {
...
uint32_t krn_visorc_start(uint32_t flag, visorc_init_params_t *sysconfig)
{
    ...
    if(sysconfig == NULL) {
        ...
        if (reg_val != 0x00000000U) {
            ...
            krn_printU5("[ERROR] krn_visorc_start() : setup register(0x%x) failed, write=0x%x read=0x%x", (uint32_t)reg, 0x0U, reg_val, 0U, 0U);
            retcode = ERR_DRV_SCHDR_VISORC_HW_UNAVAILABLE;
        }
        ...
        if(retcode == ERRCODE_NONE) {
            ...
            /* To be consistent with CV22FS although it is not necessary. */
