...
HCI_LE_Set_Advertising_Parameters ()
{
	...
	#1280 ms
	Advertising_Interval_Max="00 08"
	...
	DEVICE_NAME=`cat /usr/local/share/script/bt.conf |grep DEVICE_NAME|cut -c 13-`
...
if [ "${1}" == "leadv5" ] && [ "`cat /usr/local/share/script/bt.conf | grep GATT_PERIPHERAL=yes`" != "" ]; then
	HCI_LE_Set_Advertising_Parameters
	if [ -e /tmp/hid_gatt_disconnect_cb.sh ] && [ "`pidof gatttool`" == "" ] && [ "`pidof hcitool`" == "" ]; then
		LOGGER "To connect HOGP: /tmp/hid_gatt_disconnect_cb.sh"
