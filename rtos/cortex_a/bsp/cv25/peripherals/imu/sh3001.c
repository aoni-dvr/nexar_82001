...
{
    imu_sh3001_read(devAddr, regAddr, readBuf, readLen);
	return (SH3001_TRUE);
}
...
{
    imu_sh3001_write(devAddr, regAddr, writeBuf, writeLen);
...
unsigned char SH3001_init(void) 
{
	...
	while((regData != 0x61) && (i++ < 3))
	{
		...
		if ((i == 3) && (regData != 0x61))
		{
			debug_line("SH3001 init fail 0x%x\n", regData);
			return SH3001_FALSE;
		}	
	}
	...
	SH3001_Temp_Config(SH3001_TEMP_ODR_125, SH3001_TEMP_EN);
