...
uint32 AmbaDSP_GetMonitorConfig(uint32 ModuleId, AMBA_DSP_MONITOR_CONFIG_s *pMonCfg)
{
    ...
    if(MonitorConfigSelect == 0) {
        pMonCfg->BitMask = DSP_MON_BITMASK_LOGIC | DSP_MON_BITMASK_CRC;
    }
    if(MonitorConfigSelect == 1) {
        pMonCfg->BitMask = DSP_MON_BITMASK_LOGIC;
    }
    if(MonitorConfigSelect == 2) {
        pMonCfg->BitMask = DSP_MON_BITMASK_CRC;
    }
    if(MonitorConfigSelect == 0xff) {
        rval = -1;
    }
...
INT32 safety_stub_ctest_main(void)
{
    ...
    AmbaDSP_MonitorErrorNotify(NULL);
    MonitorConfigSelect = 0;
    AmbaDSP_GetMonitorConfig(0, &MonCfg);
    ...
    AmbaDSP_GetMonitorConfig(0, &MonCfg);
    MonitorConfigSelect = 0;
    AmbaDSP_MonitorCrcCmpr(NULL);
    AmbaUtility_Crc32(NULL, 0);
    AmbaUtility_Crc32(tmp, 10);
