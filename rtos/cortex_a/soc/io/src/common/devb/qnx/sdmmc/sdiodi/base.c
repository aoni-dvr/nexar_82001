...
extern void AmbaSD_SetHsRdLatency(UINT32 SdChanNo, UINT8 HsRdLatency);
...
int64_t sdio_parse_number64( const char *str )
{
    char    *cp;
    int64_t number;
...
int sdio_hc_getsubopt( char **optionp, char * const *tokens, char **valuep )
{
    ...
    *optionp = p;
    ...
    for( index = 0; ( token = *tokens++ ) && (len > 0); index++ ) {
...
int sdio_timing( sdio_hc_t *hc, int timing )
{
    ...
    fprintf(stderr, "%s %d bus_timing %s\r\n", __func__, __LINE__, name[timing]);
...
int sdio_options( sdio_hc_t *hc, char *options )
{
    ...
    static char *opts[] = {
        ...
        [OPTION_EMMC] = "emmc",
        [OPTION_DELAY] = "delay",
        [OPTION_HS_RD_LATENCY] = "HsRdLatency",
        NULL
    };
    ...
    while( options && *options != '\0' && status == EOK ) {
        ...
        case OPTION_DELAY:               // delay
            SDIO_ARG_VAL( opts[opt], value, status );
            AmbaSD_SetDetailDelay(sdio_parse_number( value ));
            break;

        case OPTION_HS_RD_LATENCY:               // HsRdLatency
            SDIO_ARG_VAL( opts[opt], value, status );
            AmbaSD_SetHsRdLatency(0, sdio_parse_number( value ));
            break;
...
int sdio_xpt_device_notify( sdio_hc_t *hc, int op )
{
    sdio_hc_cfg_t   *cfg;
    XPT_DEVICE      dev;
    int             idx;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/devb/sdmmc/sdiodi/base.c $ $Rev: 894852 $")
#endif
