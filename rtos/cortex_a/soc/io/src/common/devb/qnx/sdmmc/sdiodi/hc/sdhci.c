...
extern void AmbaSD_SetRdLatencyCtrl(UINT32 SdChanNo, UINT32 Value);
extern void AmbaSD_DelayCtrlSetValue(UINT32 SdChanNo, UINT32 Value);
extern UINT32 AmbaSD_GetDetailDelay(void);
extern UINT8 AmbaSD_GetHsRdLatency(UINT32 SdChanNo);
extern void AmbaSD_DelayCtrlReset(UINT32 SdChanNo);
...
static int sdhci_pwr( sdio_hc_t *hc, int vdd )
{
    ...
    if (vdd == OCR_VDD_17_195) {
        VddPower = 1U;
    } else if (vdd != 0) {
        VddPower = 2U;
    }

    AmbaUserSD_PowerCtrl(hc->Channel, VddPower);
...
#else
    UINT32 ActualFreq = 0;
    AmbaSYS_SetIoClkFreq(AMBA_CLK_SD0 + hc->Channel, clk, &ActualFreq);

    if (clk > 50000000U) {
        AmbaSD_DelayCtrlSetValue(hc->Channel, AmbaSD_GetDetailDelay());

        if (AmbaSD_GetHsRdLatency(hc->Channel) != 0) {
            AmbaSD_SetRdLatencyCtrl(hc->Channel, AmbaSD_GetHsRdLatency(hc->Channel));
        }

    } else {
        AmbaSD_DelayCtrlReset(hc->Channel);
    }
#endif
    // enable internal clock & enable clock to the card
    sctl |= SDHCI_SYSCTL_CLK( clkd ) | SDHCI_SYSCTL_ICE | SDHCI_SYSCTL_CEN;
    sdhci_out32( base + SDHCI_SYSCTL, sctl );
...
static int sdhci_signal_voltage( sdio_hc_t *hc, int signal_voltage )
{
    ...
    if( signal_voltage == SIGNAL_VOLTAGE_3_3 ) {
        ...
        if( !( sdhci_in32( base + SDHCI_PSTATE ) & SDHCI_PSTATE_DLSL_MSK ) || (hc->flags & HC_FLAG_DEV_MMC) ) {
            ...
            UINT32 ActualFreq;
            AmbaSYS_SetIoClkFreq(AMBA_CLK_SD0, 25000000, &ActualFreq);
            ...
            if( ( sdhci_in16( base + SDHCI_HCTL2 ) & SDHCI_HCTL2_SIG_1_8V ) ) {
                ...
                uint32_t Retry = 0;
                while ((sdhci_in32( base + SDHCI_PSTATE ) & SDHCI_PSTATE_DLSL_MSK ) != SDHCI_PSTATE_DLSL_MSK) {
...
static int sdhci_cd( sdio_hc_t *hc )
{
    ...
    if( ( pstate & SDHCI_PSTATE_CI ) ) {
        if( sdhci_waitmask( hc, SDHCI_PSTATE, SDHCI_CARD_STABLE, SDHCI_CARD_STABLE, 100000 ) == EOK ) {
            ...
            /* Ambarella: Bit19 = 1 = WP */
            if ((pstate & SDHCI_PSTATE_WP) != 0) {
                cstate  |= CD_WP;
            }
        }
    }
...
#define BASECLK        4
    ...
    while( options && *options != '\0' ) {
        ...
        case HS200:
            ...
            break;
        case BASECLK:
            hc->clk_max    = strtoul(value, NULL, 0);
            break;
        }
    }
...
int sdhci_init( sdio_hc_t *hc )
{
    ...
    if (cfg->base_addr[0] == AMBA_CA53_SD_BASE_ADDR) {
        hc->Channel = AMBA_SD_CHANNEL0;
    } else {
        hc->Channel = AMBA_SD_CHANNEL1;
    }
#else
    if (cfg->base_addr[0] == AMBA_CORTEX_A53_SD0_BASE_ADDR) {
        hc->Channel = AMBA_SD_CHANNEL0;
    } else {
        hc->Channel = AMBA_SD_CHANNEL1;
    }
    ...
    hc->clk_max = hc->clk_max == 0 ? SDHCI_CAP_BASE_CLK( cap, hc->version ) : hc->clk_max;
...
//        hc->caps    |= HC_CAP_HS200;
    ...
    hc->caps |= HC_CAP_SDR50 | HC_CAP_SDR25 | HC_CAP_SDR12 | HC_CAP_SDR104;
    ...
    if( ( cap & SDHCI_CAP_DMA ) ) {
        ...
        if (0) { /* Ambarella, FIXME: Only use SDMA. */
    ...
    UINT32 Freq = 0;
    AmbaSYS_SetClkFreq(AMBA_SYS_CLK_SD, 600000000U, &Freq);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/devb/sdmmc/sdiodi/hc/sdhci.c $ $Rev: 895347 $")
#endif
