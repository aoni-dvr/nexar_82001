...
extern void AmbaSD_SetDetailDelay(UINT32 Value);
...
int sdmmc_partition_enum( SIM_HBA *hba )
{
    ...
    for( pidx = 0; pidx < MMC_GP_PART_MAX; pidx++, part++ ) {
        ...
        gp_size |= (uint64_t)ecsd[ECSD_GP_SIZE + 1 + ( pidx * 3 ) ] << 8U;
        gp_size |= (uint64_t)ecsd[ECSD_GP_SIZE + 2 + ( pidx * 3 ) ] << 16U;
        nlba = ( gp_size * wp_grp_size ) / 512U;

        part            = &ext->targets[ 4 + pidx].partitions[0];
        part->config    = (UINT32)(pconf | ( 4 + pidx ));
        part->slba        = 0;
        part->nlba        = nlba;
        part->elba        = nlba ? ( part->nlba - 1U ) : 0;
...
int sdmmc_dev_dcfg( SIM_HBA *hba )
{
    SIM_SDMMC_EXT    *ext;
    int                status;
...
//    cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s:  hba %p", __FUNCTION__, hba );
    ...
    if( hba->coid != (UINT32) -1 ) {
        ConnectDetach( hba->coid );
    }

    if( hba->chid != (UINT32) -1 ) {
...
int sdmmc_reg( SIM_HBA *hba )
{
    ...
    if( (UINT32)hba->pathid > sdmmc_ctrl.pathid_max ) {
        sdmmc_ctrl.pathid_max = (UINT32)hba->pathid;
    }
...
int sdmmc_sense( CCB_SCSIIO *ccb, int sense, int asc, int ascq )
{
    ...
    ccb->cam_scsi_status    = (UINT8)SCS_CHECK;
    ccb->cam_ch.cam_status    = (UINT8)CAM_REQ_CMP_ERR;
...
int sdmmc_mpage_rw_err( SIM_HBA *hba, CCB_SCSIIO *ccb, int op )
{
    ...
    if( op ) {
        ...
        if( ( ext->targets[ccb->cam_ch.cam_target_id].partitions[ccb->cam_ch.cam_target_lun].pflags & SDMMC_PFLAG_WP ) ) {
            mph->device_specific |= (UINT8)MP_DS_WP;
        }
        status = CAM_REQ_CMP;
    }
...
int sdmmc_std_inquiry( SIM_HBA *hba, CCB_SCSIIO *ccb )
{
    ...
    iptr->adlen            = 96U - 5U;    // nbytes after adlen field

    strlcpy( (char *)iptr->vend_id, "SDMMC:", sizeof( iptr->vend_id ) );
    strlcpy( (char *)iptr->prod_id, (char *)ext->dev_inf.pnm, sizeof( iptr->prod_id ) );

    if( ( ext->dev_inf.prv >> 4U ) < 10U )
        iptr->prod_rev[0] = ( ext->dev_inf.prv >> 4U ) + '0';
    else
        iptr->prod_rev[0] = ( ext->dev_inf.prv >> 4U ) - 10U + 'a';

    iptr->prod_rev[1] = '.';

    if( ( ext->dev_inf.prv & 0x0F) < 10U )
        iptr->prod_rev[2] = ( ext->dev_inf.prv & 0x0FU ) + '0';
    else
        iptr->prod_rev[2] = ( ext->dev_inf.prv & 0x0FU ) - 10U + 'a';
...
int sdmmc_ffu( SIM_HBA *hba, CCB_SCSIIO *ccb )
{
    SIM_SDMMC_EXT    *ext;
    SDMMC_PARTITION    *part;
    sdio_ecsd_t        *ecsd;
    ...
    if( ( ccb->cam_ch.cam_flags & (UINT32)CAM_SCATTER_VALID ) ) {
...
int sdmmc_pm( SIM_HBA *hba, int op )
{
    ...
    case PM_IDLE:
        ...
        if( ( ext->hc_inf.caps & (UINT32)HC_CAP_SLEEP ) ) {
    ...
    case PM_ACTIVE:
        if( ext->pm_state == (UINT32)PM_SLEEP ) {
            if( ( ext->dev_inf.caps & DEV_CAP_ASSD ) ) {
                atomic_set( &ext->eflags, SDMMC_EFLAG_ASSD_INIT );
            }
        }
...
int sdmmc_pwroff_notify( SIM_HBA *hba, uint8_t cfg )
{
    ...
    if( ( status = sdio_send_cmd( ext->device, cmd, NULL, SDIO_TIME_DEFAULT, 0 ) ) != EOK ) {
        ...
        if( (cstatus == (UINT32)CS_CMD_TO_ERR) && (cfg == (UINT32)ECSD_POWER_OFF_LONG) ) {
            ecsd = sdio_get_raw_ecsd( ext->device );
...
int sdmmc_bkops( SIM_HBA *hba, int tick )
{
    ...
    case BKOPS_STATUS_OPERATIONS_CRITICAL:
        if( sdio_mmc_switch( ext->device, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, ECSD_BKOPS_START, ECSD_BKOPS_INITIATE, SDIO_BKOPS_MAX_TIMEOUT ) == EOK ) {
...
int sdmmc_write_protect( SIM_HBA *hba, int op, int partition, int mode, uint32_t lba, uint32_t nlba, uint64_t *prot )
{
    ...
    dev            = ext->device;
    blksz        = ext->dev_inf.sector_size;
    wp_grp_size = ext->dev_inf.wp_size / 512U;
    ...
    case MMC_CLR_WRITE_PROT:
        while( nlba ) {
            addr = ( ext->dev_inf.caps & (UINT64)DEV_CAP_HC ) ? lba : ( (uint64_t)lba * blksz );
    ...
    case MMC_SEND_WRITE_PROT_TYPE:
        ...
        sdio_setup_cmd_io( cmd, (UINT32)SCF_DIR_IN, 1, sz, &sge, 1, NULL );
        if( ( status = sdio_send_cmd( dev, cmd, NULL, SDMMC_TIME_DEFAULT * 1000, 0 ) ) == EOK ) {
...
int sdmmc_set_partition_attr( SIM_HBA *hba, int partition, int flags )
{
    ...
    for( tidx = 0; tidx <= ext->ntargs; tidx++ ) {
        ...
        for( pidx = 0; pidx <= targ->nluns; pidx++ ) {
            ...
            if( ( part->config & (UINT32)MMC_PART_MSK ) == (UINT32)partition ) {
                part->pflags |= flags;
            }
        }
    }
...
int sdmmc_rw( SIM_HBA *hba, SDMMC_PARTITION *part, int flgs, uint32_t addr, int dlen, sdio_sge_t *sgl, int sgc, void *mhdl, uint32_t timeout )
{
    ...
    blks    = dlen / blksz;
    addr    = ( di->caps & (UINT64)DEV_CAP_HC ) ? addr : ( addr * blksz );
    op        = ( flgs & SCF_DIR_IN ) ? MMC_READ_SINGLE_BLOCK : MMC_WRITE_BLOCK;

    if( dlen > blksz ) {
        if( !( ext->hc_inf.caps & (UINT64)HC_CAP_ACMD12 ) ) {
            if( ( di->caps & (UINT64)DEV_CAP_CMD23 ) ) {
                flgs |= SCF_SBC;
            }
        }
        flgs |= SCF_MULTIBLK;
    ...
    if( status ) {
        ...
        if( status == ETIMEDOUT && ( flgs & SCF_SBC ) ) {
            ...
            if( ext->instance.ident.dtype == (UINT16)DEV_TYPE_SD ) {
                di->caps &= ~(UINT64)DEV_CAP_CMD23;
    ...
    //fprintf(stderr, "%s:  flgs 0x%x, lba %d, len %d, status 0x%x, cstatus 0x%x, rsp[0] 0x%x\r\n",
    //            __FUNCTION__, flgs, addr, dlen, status, cstatus, rsp[0] );

    if (iostat[0].enable != 0U) {
        UINT32 EndTime = 0, TimeDiff;
        ...
        if ((flgs & SCF_DIR_IN) != 0) {
            iostat[0].rd_time  += TimeDiff;
            iostat[0].rd_bytes += (UINT64)dlen;
            prf2_iostat_diagram(iostat[0].rlvl, dlen / 512U);
        } else {
            iostat[0].wr_time+= TimeDiff;
            iostat[0].wr_bytes += (UINT64)dlen;
            prf2_iostat_diagram(iostat[0].wlvl, dlen / 512U);
        }
    }
...
int sdmmc_write_verify( SIM_HBA *hba, CCB_SCSIIO *ccb )
{
    ...
    if( ccb->cam_dxfer_len > (UINT32)SDMMC_VER_BSIZE ) {
...
int sdmmc_read_write( SIM_HBA *hba, CCB_SCSIIO *ccb, int flgs )
{
    ...
    if( ( ccb->cam_ch.cam_flags & (UINT32)CAM_DATA_PHYS ) ) {
        flgs |= SCF_DATA_PHYS;
    }
...
int sdmmc_drvr_state_devctl( SIM_HBA *hba, CCB_DEVCTL *ccb )
{
    SIM_SDMMC_EXT        *ext;
    SDMMC_DRVR_STATE    *ds;
    ...
    if( ds->state == (UINT32)SDMMC_DRVR_PAUSE ) {
        ...
        ccb->cam_ch.cam_flags &= ~(UINT32)CAM_SIM_QFRZDIS;
        ccb->cam_ch.cam_flags |= (UINT32)CAM_SIM_QFREEZE;
...
int sdmmc_wp_devctl( SIM_HBA *hba, CCB_DEVCTL *ccb )
{
    ...
    if( sdmmc_unit_ready( hba, (CCB_SCSIIO *)ccb ) != CAM_REQ_CMP ) {
        status = EIO;
    } else if( !op || slba > pend || ( slba + nlba - 1U ) > pend ) {
        status = EINVAL;
    } else if( ext->instance.ident.dtype != (UINT16)DEV_TYPE_MMC ) {
        status    = ENOTSUP;
...
int sdmmc_erase_devctl( SIM_HBA *hba, CCB_DEVCTL *ccb )
{
    ...
    if( sdmmc_unit_ready( hba, (CCB_SCSIIO *)ccb ) != CAM_REQ_CMP ) {
        status = EIO;
    } else if( slba > pend || ( slba + nlba - 1U ) > pend ) {
...
int sdmmc_lock_unlock_devctl( SIM_HBA *hba, CCB_DEVCTL *ccb )
{
    ...
    if( sdmmc_unit_ready( hba, (CCB_SCSIIO *)ccb ) != CAM_REQ_CMP ) {
        ccb->cam_devctl_status = EIO;
        return( CAM_REQ_CMP );
    } else if( lock->pwd_len > (UINT32)SDMMC_LU_PWD_SIZE ||
               ( lock->action != (UINT32)SDMMC_LU_ACTION_LOCK &&
                 lock->action != (UINT32)SDMMC_LU_ACTION_SET &&
                 lock->action != (UINT32)SDMMC_LU_ACTION_CLR &&
                 lock->action != (UINT32)SDMMC_LU_ACTION_UNLOCK &&
                 lock->action != (UINT32)SDMMC_LU_ACTION_ERASE ) ) {
...
int sdmmc_pwr_mgnt_devctl( SIM_HBA *hba, CCB_DEVCTL *ccb )
{
    ...
    if( sdmmc_unit_ready( hba, (CCB_SCSIIO *)ccb ) != CAM_REQ_CMP ) {
        ...
        case SDMMC_PM_ACTION_GET:
            pm->idle_time    = ext->pm_idle_time_ns / 1000000U;
            pm->sleep_time    = ext->pm_sleep_time_ns / 1000000U;
            break;
...
static int sdmmc_rpmb_info( SIM_HBA *hba, CCB_DEVCTL *ccb, RPMB_INFO *info, int minsz )
{
    ...
    if( ccb->cam_devctl_size < (UINT64)minsz ) {
    ...
    info->flags     = ( csd->write_protect & 0x01U ) ? RPMB_FLAG_WP : 0;
...
static int sdmmc_rpmb_devctl( SIM_HBA *hba, CCB_DEVCTL *ccb )
{
    ...
    nf = req->flags == (UINT32)RPMB_FLG_READ ? req->nframes : 1U;
...
static int sdmmc_gencmd_devctl( SIM_HBA *hba, CCB_DEVCTL *ccb )
{
    ...
    for( ; ; ) {
        ...
        if( ( req->flags & (UINT32)SDMMC_GENCMD_DIR ) == (UINT32)SDMMC_GENCMD_READ ) {
            flags |= (UINT32)SCF_DIR_IN;
        } else {
            flags |= (UINT32)SCF_DIR_OUT;
        }
...
static int sdmmc_mancmd_devctl( SIM_HBA *hba, CCB_DEVCTL *ccb )
{
    ...
    for( ; ; ) {
        ...
        if( ( req->flags & SDMMC_MANCMD_DIR ) == SDMMC_MANCMD_READ ) flags |= (UINT32)SCF_DIR_IN;
        if( ( req->flags & SDMMC_MANCMD_RSP ) == SDMMC_MANCMD_R1 ) flags |= SCF_RSP_R1;
        else flags |= SCF_RSP_R1B;
...
static int sdmmc_set_config( SIM_HBA *hba, CCB_DEVCTL *ccb )
{
    AMBA_SD_SETTING_s   *pSdSetting;

    pSdSetting        = (AMBA_SD_SETTING_s *)ccb->cam_devctl_data;
#if 0
    int                    status;
    if( sdmmc_unit_ready( hba, (CCB_SCSIIO *)ccb ) != CAM_REQ_CMP ) {
        ccb->cam_devctl_status = EIO;
    ...
    (void)hba;
#endif
    AmbaSD_SetDetailDelay(pSdSetting->DetailDelay);

    //AmbaSD_SetHsRdLatency(0, pSdSetting->HsRdLatency);
...
static int sdmmc_iostat_info( SIM_HBA *hba, CCB_DEVCTL *ccb )
{
    io_statistics_t        *pIoStat;
    (void)hba;

    pIoStat = (io_statistics_t *)ccb->cam_devctl_data;
    ccb->cam_devctl_status = EOK;

    iostat[0].enable = pIoStat->enable;

    memcpy(pIoStat, &iostat[0], sizeof( io_statistics_t ) );
...
int sdmmc_devctl( SIM_HBA *hba, CCB_DEVCTL *ccb )
{
    ...
    case DCMD_SDMMC_RESET:
        (void) sdmmc_reset( hba );
        ccb->cam_devctl_status    = EOK;
        status = CAM_REQ_CMP;
        break;

    case DCMD_SDMMC_SET_CONFIG:
        status = sdmmc_set_config( hba, ccb );
        break;

    case DCMD_SDMMC_IOSTAT_INFO:
        status = sdmmc_iostat_info( hba, ccb );
        break;
...
void *sdmmc_driver_thread( void *hdl )
{
    ...
    if( ( hba->chid = ChannelCreate( _NTO_CHF_DISCONNECT | _NTO_CHF_UNBLOCK ) ) == (UINT32) -1 ) {
        cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s ChannelCreate failure %s", __FUNCTION__, strerror( errno ) );
        stat = CAM_TRUE;
    }

    if( !stat && ( hba->coid = ConnectAttach( 0, 0, hba->chid, _NTO_SIDE_CHANNEL, 0 ) ) == (UINT32) -1 ) {
...
int sdmmc_path_inq( SIM_HBA *hba, CCB_PATHINQ *ccb )
{
    ...
    ccb->cam_version_num    = (UINT8)CAM_VERSION;
    ccb->cam_initiator_id    = 7;
    ccb->cam_hba_inquiry    = 0;
    ccb->cam_target_sprt    = 0;
    ccb->cam_hba_misc        = 0;
    ccb->cam_hba_eng_cnt    = 0;
    ccb->cam_sim_priv        = (UINT32)SIM_PRIV;
    ccb->cam_async_flags    = (UINT32)AC_BUS_RESET;
    ...
    ccb->cam_vuhba_flags[CAM_VUHBA_MAX_LINKED]    = 1U;
...
int sdmmc_sim_action_devctl( SIM_HBA *hba, CCB *ccb )
{
    SIM_SDMMC_EXT    *ext;
    CCB_HEADER        *ch;
    CCB_DEVCTL      *dccb;
    ...
    if( dccb->cam_devctl_dcmd == (UINT32)DCMD_SDMMC_DRVR_STATE ) {
        SDMMC_DRVR_STATE    *ds;
...
int sdmmc_sim_attach( CAM_ENTRY *centry )
{
    ...
    if( ( sdmmc_ctrl.cflags & SDMMC_CFLAG_SCAN ) ) {
        ...
        for( hba = TAILQ_FIRST( &sdmmc_ctrl.hlist ); hba; hba = nhba ) {
            ...
            ext                    = (SIM_SDMMC_EXT *)hba->ext;
            inst                = &ext->instance;
            inst->ident.vid        = hba->cfg.Device_ID.DevID & 0xffffU;
            inst->ident.did        = hba->cfg.Device_ID.DevID >> 16U;
...
int sdmmc_sim_args( char *options )
{
    ...
    if( ext->instance.path == (UINT32)SDIO_CONNECT_WILDCARD ) {
        sdmmc_ctrl.cflags |= (UINT32)SDMMC_CFLAG_SCAN;
    }
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/devb/sdmmc/sim_sdmmc.c $ $Rev: 894852 $")
#endif
