...
i2c_status_t Amba_I2c_send(void *hdl, void *buf, unsigned int len, unsigned int stop)
{
    ...
    dev->Txbuf = (uint8_t*)buf;
    dev->stop = stop;

    TxTransaction.DataSize = len;
    TxTransaction.pDataBuf = (unsigned char *)buf;
    TxTransaction.SlaveAddr = dev->slave_addr;

    if (dev->stop) {
        AmbaRTSL_I2cWrite(dev->ChannelId, (dev->stop << 0x4) | dev->speed, &TxTransaction);
