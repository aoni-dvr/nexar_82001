...
int main(int argc, char *argv[])
{
    ...
    if (!dpp) {
        slogf(_SLOG_SETCODE(_SLOGC_CHAR, 0), _SLOG_INFO,
              "RTC error: Failed to create dispatch interface\n");
        goto fail;
    }
    ...
    io_funcs.devctl = rtc_io_devctl;
    iofunc_attr_init(&ioattr, S_IFCHR | 0666, NULL, NULL);

    /* Attach the device name */
    id = resmgr_attach(dpp, &rattr, "/dev/rtc", _FTYPE_ANY, 0,
                       &connect_funcs, &io_funcs, &ioattr);
    if (id == -1) {
        slogf(_SLOG_SETCODE(_SLOGC_CHAR, 0), _SLOG_INFO,
              "RTC error: Failed to attach pathname\n");
        goto fail;
    }
    ...
    if (0x0 != rtc_dev_init()) {
        fprintf(stderr, "rtc_dev init fail %s(%d)\n", __func__, __LINE__);
    }
    ...
    printf("rtc driver init done\n");

    while (1) {
        if ((ctp = dispatch_block(ctp)) == NULL) {
            slogf(_SLOG_SETCODE(_SLOGC_CHAR, 0), _SLOG_INFO,
                  "RTC error: Block error\n");
            goto fail;
        }
...
int rtc_io_devctl(resmgr_context_t *ctp, io_devctl_t *msg, RESMGR_OCB_T *ocb)
{
    int status, nbytes;
    int err = EOK;
    ...
    case DCMD_RTC_SET:
        rtc_msg = _DEVCTL_DATA(msg->i);
        AmbaRTSL_RtcSetSystemTime(rtc_msg->second);
        break;

    case DCMD_RTC_GET:
        rtc_msg = _DEVCTL_DATA(msg->i);
        rtc_msg->second = AmbaRTSL_RtcGetSystemTime();
        nbytes = sizeof(rtc_msg_t);
        break;
    }
