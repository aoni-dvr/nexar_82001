...
int main(int argc, char *argv[])
{
    ...
    if (verbose >= 1) {
        for (CLockId = 0; CLockId < AMBA_NUM_PLL; CLockId++) {
            ...
            fprintf(stderr, "Clock:%s actual frequency %uHz\n", ClkIdx2String(CLockId), Amba_Clk_Get_Pll_Freq(CLockId));
        }
    }
...
int clk_io_devctl(resmgr_context_t *ctp, io_devctl_t *msg, RESMGR_OCB_T *ocb)
{
    ...
    case DCMD_CLOCK_SET_PLL_FREQ:
        ...
        freq = pll_freq->freq;
        if (Amba_Clk_Set_Pll_Freq(pll_freq->id, freq) != 0)
            err = EINVAL;
        break;
    ...
    case DCMD_CLOCK_CONFIG_VOUT:
        pVoutInfo = _DEVCTL_DATA(msg->i);
        if (pVoutInfo->device == 0x0) {
            AmbaRTSL_PllSetVoutLcdClkHint(pVoutInfo->Type, pVoutInfo->Ratio);
        } else if (pVoutInfo->device == 0x1) {
            AmbaRTSL_PllSetVoutTvClkHint(pVoutInfo->Type, pVoutInfo->Ratio);
        } else {
            /* incorrect device id, do nothing */
        }
        break;
