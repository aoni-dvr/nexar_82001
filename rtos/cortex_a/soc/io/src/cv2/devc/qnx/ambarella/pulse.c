...
static void * my_pulse_thread (void *args)
{
    ...
    for (;;) {
        if (MsgReceivePulse (mps->chid, &pulse, sizeof (pulse), NULL) != -1) {
            ...
            slogf(_SLOG_SETCODE(_SLOGC_CHAR, 0), _SLOG_ERROR, "%s: MsgReceivePulse failed: %s", __FUNCTION__, strerror(errno));
        }
    }
                     ...
                     void (*handler) (DEV_UART * dev, struct sigevent * event), DEV_UART * dev)
{
    ...
    if ((pthread_create (&mps->tid, &attr, my_pulse_thread, mps)) != EOK) {
        slogf(_SLOG_SETCODE(_SLOGC_CHAR, 0), _SLOG_ERROR, "%s: pthread_create failed: %s", __FUNCTION__, strerror(errno));
        return errno;
    }
