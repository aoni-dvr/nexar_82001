...
{
    printf("diag-ambarella [i2c|gpio|timer|rng|hdmi|rtc|adc|otp|spi|mprint|dma|gdma|cvbs|crc]\n");
}
...
static int parse_options(int argc, char *argv[])
{
    ...
    if (argc >= 2) {
        ...
        if (strncmp(argv[1], "hdmi", 4U) == 0) {
            DoHdmiDiag(argc, argv);
        }
        ...
        if (strncmp(argv[1], "cvbs", 4U) == 0) {
            DoCvbsDiag(argc, argv);
        }
        ...
        if (strncmp(argv[1], "dram", 4U) == 0) {
            DoDramShmooDiag(argc, argv);
        }
    } else {
