...
int main(int argc, char *argv[])
{
    ...
    io_funcs.devctl = dram_io_devctl;
    iofunc_attr_init(&ioattr, S_IFCHR | 0666, NULL, NULL);

    /* Attach the device name */
    id = resmgr_attach(dpp, &rattr, "/dev/dramc", _FTYPE_ANY, 0,
...
{
    int status, nbytes;
    AMBA_DRAMC_STATIS_s *pStatisData = {0};
    int err = EOK;
    ...
    nbytes = 0;
    switch(msg->i.dcmd) {
    case DCMD_DRAM_RESET:
        (void)AmbaRTSL_DramcEnableStatisCtrl();
        break;

    case DCMD_DRAM_GET_STATIS:
        pStatisData = _DEVCTL_DATA(msg->i);
        (void)AmbaRTSL_DramcGetStatisInfo(pStatisData);
        nbytes = sizeof(AMBA_DRAMC_STATIS_s);
        break;
    }
