...
int hdmi_dev_init(void)
{
    ...
    if ((VirtualAddr = ioremap(AMBA_SCRATCHPADS_BASE_ADDR,
                                       AMBA_SCRATCHPADS_MMAP_SIZE)) == NULL) {
        pr_err("%s: cannot access to rct region\n", __func__);
        return -1;
    } else {
    ...
    if ((VirtualAddr = ioremap(AMBA_VOUT_DISPLAY1_CONFIG_BASE_ADDR,
                                       AMBA_VOUT_DISPLAY1_CONFIG_MMAP_SIZE)) == NULL) {
        pr_err("%s: cannot access to rct region\n", __func__);
        return -1;
    } else {
        pAmbaVoutDisplay1_Reg = (AMBA_VOUT_DISPLAY1_CONFIG_REG_s *) VirtualAddr;
    }
    ...
    if ((VirtualAddr = ioremap(AMBA_VOUT_DISPLAY1_CONFIG_BASE_ADDR,
        ...
        pAmbaVoutTvEnc_Reg = (AMBA_VOUT_TVENC_CONTROL_REG_s*) VirtualAddr;
    }
...
int hdmi_dev_dinit(void)
{
    ...
    iounmap((void __iomem*)pAmbaScratchpadS_Reg);
    iounmap((void __iomem*)pAmbaVoutTop_Reg);
    iounmap((void __iomem*)pAmbaVoutDisplay1_Reg);
...
UINT32 hdmi_TxSetMode(UINT32 HdmiPort, const AMBA_HDMI_VIDEO_CONFIG_s *pVideoConfig, const AMBA_HDMI_AUDIO_CONFIG_s *pAudioConfig, AMBA_HDMI_VIDEO_CONFIG_s *pWorkingVideoConfig)
{
    ...
    if ((HdmiPort >= NUM_HDMI_TX_PORT) || (pVideoConfig == NULL)) {
        ...
        if (RetVal == 0) {
            AmbaRTSL_HdmiReset(1U);
            AmbaRTSL_HdmiReset(0U);
...
{
    //const AMBA_HDMI_VIDEO_CONFIG_s *pVideoConfig = &AmbaHdmiCtrl.VideoCfg;
    //UINT32 HotPlugDetect, EdidDetect = HDMI_EDID_HDMI;
    UINT32 RetVal = 0;
