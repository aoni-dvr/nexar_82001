...
void create_device(TTYINIT_AMBA *dip, unsigned unit)
{
    ...
    // assign base address to all UART instance
    // it is to fit RTSL design
    for (i = 0; i < AMBA_NUM_UART_CHANNEL; i++) {
        pAmbaUART_Reg[i] = (AMBA_UART_REG_s *)dev->base;
    }
    ...
    */
    if (dip->tty.port != 0 && dev->intr[0] != 0xFFFF) {
        // always use AMBA_UART_APB_CHANNEL0 to use non-DMA mode.
