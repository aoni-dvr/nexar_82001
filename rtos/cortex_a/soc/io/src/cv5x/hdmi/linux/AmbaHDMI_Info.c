...
static AMBA_HDMI_SINK_INFO_s AmbaHdmiSinkInfo;
static UINT32 AudioClkFreq = 12288000U; /* Default HDMI audio clock value */

/**
 *  hdmi_TxCopySinkInfo - Duplicate sink info data structure
...
UINT32 hdmi_TxCopySinkInfo(AMBA_HDMI_SINK_INFO_s *pSinkInfo)
{
    ...
    if (pSinkInfo == NULL) {
        ...
        OSAL_memcpy(&pDst, &pSinkInfo, sizeof(pSinkInfo));
        OSAL_memcpy(&pSrc, &pSrcSinkInfo, sizeof(pSrcSinkInfo));
        Size = sizeof(AMBA_HDMI_SINK_INFO_s) & ~0x3U;
        while (Size != 0U) {
            *pDst = *pSrc;
            pDst++;
            pSrc++;
...
static UINT32 HDMI_ParseHdmi14bVSDB(const UINT8 *pHdmiVSDB)
{
    ...
    if ((pHdmiVSDB[8] & HDMI_VSDB_HDMI_VIDEO_PRESENT) != 0x00U) {
        ...
        OSAL_memcpy(&VicDesc, &AmbaHdmiSinkInfo.VideoInfoBuf[AmbaHdmiSinkInfo.NumVideoInfo], sizeof(AMBA_HDMI_VIDEO_DESCRIPTOR_s));
        for (i = 0U; i < LengthVIC; i ++) {
            ...
            if (VicDesc.Valid2D == 1U) {
                OSAL_memcpy(&AmbaHdmiSinkInfo.VideoInfoBuf[AmbaHdmiSinkInfo.NumVideoInfo], &VicDesc, sizeof(AMBA_HDMI_VIDEO_DESCRIPTOR_s));
...
static void HDMI_ParseDTD(const UINT8 *pDescriptor)
{
    ...
    AmbaHdmiSinkInfo.VideoInfoBuf[AmbaHdmiSinkInfo.NumVideoInfo] = 0U;
    OSAL_memcpy(&VicDesc, &AmbaHdmiSinkInfo.VideoInfoBuf[AmbaHdmiSinkInfo.NumVideoInfo], sizeof(AMBA_HDMI_VIDEO_DESCRIPTOR_s));
    VicDesc.FormatID = HDMI_VIC_DTD0 + (UINT16)AmbaHdmiSinkInfo.NumDTD;
...
static UINT32 HDMI_ParseCea861Tag(const UINT8 *pDataBlock, UINT32 Tag, UINT32 Length)
{
    ...
    case EDID_CEA861_TAG_VIDEO:
        for (i = 0U; i < Length; i ++) {
            ...
            if (VIC[i] <= 64U) {
                VicDesc.Native = pDataBlock[i + 1U] >> 7U;
            } else {
    ...
    case EDID_CEA861_TAG_AUDIO:
        for (i = 0U; i < Length; i += 3U) {
            ...
            OSAL_memcpy(&AicDesc, &AmbaHdmiSinkInfo.AudioInfoBuf[AIC], sizeof(AMBA_HDMI_AUDIO_DESCRIPTOR_s));
            if (AIC == 0xFU) {
            ...
            OSAL_memcpy(&AmbaHdmiSinkInfo.AudioInfoBuf[AIC], &AicDesc, sizeof(AMBA_HDMI_AUDIO_DESCRIPTOR_s));
        }
        break;
...
static UINT32 HDMI_ParseCea861Extension(const EDID_CEA861_EXT_V3_FORMAT_s *pCea861Edid)
{
    ...
    for (i = 0U; (i < 6U) && ((Offset + 18U) <= 123U); i ++) {
        if ((pCea861Edid->Data[Offset] == 0U) && (pCea861Edid->Data[Offset + 1U] == 0U)) {
            /* It is not a detailed timing descriptor */
            if ((pCea861Edid->Data[Offset + 2U] == 0U) &&
                (pCea861Edid->Data[Offset + 3U] == 0U) &&
                (pCea861Edid->Data[Offset + 4U] == 0U) &&
                (pCea861Edid->Data[Offset + 5U] == 0U) &&
                (pCea861Edid->Data[Offset + 6U] == 0U) &&
                (pCea861Edid->Data[Offset + 7U] == 0U) &&
                (pCea861Edid->Data[Offset + 8U] == 0U) &&
                (pCea861Edid->Data[Offset + 9U] == 0U) &&
                (pCea861Edid->Data[Offset + 10U] == 0U) &&
                (pCea861Edid->Data[Offset + 11U] == 0U) &&
                (pCea861Edid->Data[Offset + 12U] == 0U) &&
                (pCea861Edid->Data[Offset + 13U] == 0U) &&
                (pCea861Edid->Data[Offset + 14U] == 0U) &&
                (pCea861Edid->Data[Offset + 15U] == 0U) &&
                (pCea861Edid->Data[Offset + 16U] == 0U) &&
                (pCea861Edid->Data[Offset + 17U] == 0U)) {
                /* Unused descriptor locations in CEA Extension */
                break;
            }
        } else {
            HDMI_ParseDTD(&pCea861Edid->Data[Offset]);

            if (NumOfNativeDTD > 0U) {
    ...
    if ((pCea861Edid->NumOfDTD & EDID_CEA861_BASIC_AUDIO_SUPPORTED) != 0U) {
        OSAL_memcpy(&AicDesc, &AmbaHdmiSinkInfo.AudioInfoBuf[HDMI_AIC_LPCM], sizeof(AMBA_HDMI_AUDIO_DESCRIPTOR_s));
        ...
        OSAL_memcpy(&AmbaHdmiSinkInfo.AudioInfoBuf[HDMI_AIC_LPCM], &AicDesc, sizeof(AMBA_HDMI_AUDIO_DESCRIPTOR_s));
    }
...
/**
 *  hdmi_TxParseEDID - Parse EDID to check if the display device supports features specific to HDMI
 *  @param[in] pEDID EDID buffer
 *  @param[out] pEdidDetect Indicate if the HDMI tag is found in EDID
 *  @return error code
 */
UINT32 hdmi_TxParseEDID(const UINT8 pEDID[EDID_NUM_BLOCKS][EDID_BLOCK_SIZE], UINT32 *pEdidDetect)
{
    ...
    (void)AmbaWrap_memset(&AmbaHdmiSinkInfo, 0x0, sizeof(AMBA_HDMI_SINK_INFO_s));

    pEdidBlock = pEDID[0];
    OSAL_memcpy(&pEdidStruct, &pEdidBlock, sizeof(pEdidBlock));
    ...
    if (!((pEdidStruct->VersionNumber == 1U) && (pEdidStruct->RevisionNumber >= 3U))) {
        ...
        for (i = 1U; i <= pEdidStruct->ExtensionBlockCount; i ++) {
            ...
            if (pEdidBlock[0] == EDID_EXT_CEA) {
                OSAL_memcpy(&pEdidCea861Struct, &pEdidBlock, sizeof(pEdidBlock));
                if (HdmiSupport == 0U)  {
...
/**
 *  hdmi_AudioChannelSelect - Selects target HDMI audio channel to be programmed
 *  @param[in] HDMIAudioChannel Target vout channel
 *  @param[in] AudioClk Target audio clock frequency
 *  @return error code
...
int hdmi_AudioChannelSelect(UINT32 HDMIAudioChannel, UINT32 AudioClk)
{
    ...
    AudioClkFreq = AudioClk;

    return 0;
}

/**
 *  hdmi_TxConfigAudio - Configure audio
...
/**
 *  hdmi_TxSendAudioInfoFrame - Fill Audio InfoFrame Packet
...
{
    AMBA_HDMI_PACKET_HEADER_s PktHeader;
    UINT32 NumAudioChan;
    UINT32 RetVal = HDMI_ERR_NONE;
    ...
    if (pAudioConfig != NULL) {
        ...
        if (NumAudioChan > 0U) {
            ...
            OSAL_memcpy(&pPacket->Header, &PktHeader, sizeof(PktHeader));
...
/**
 *  hdmi_TxSendAviInfoFrame - Fill AVI InfoFrame Packet
 *  @param[in] pPacket pointer to a packet buffer
 *  @param[in] pVideoConfig pointer to video format configuration
 *  @param[in] TxMode Packet transmission frequency
 *  @return error code
...
/**
 *  hdmi_TxSendSpdInfoFrame - Fill SPD InfoFrame Packet
 *  @param[in] pPacket pointer to a packet buffer
...
/**
 *  hdmi_TxSendVendorInfoFrame - Fill Vendor-Specific InfoFrame Packet
 *  @param[in] pPacket pointer to a packet buffer
 *  @param[in] pVideoConfig pointer to video format configuration
 *  @param[in] TxMode Packet transmission frequency
 *  @return error code
...
UINT32 hdmi_TxSendVendorInfoFrame(AMBA_HDMI_DATA_ISLAND_PACKET_s *pPacket, const AMBA_HDMI_VIDEO_CONFIG_s *pVideoConfig, UINT32 TxMode)
{
    ...
    if (pVideoConfig != NULL) {
        OSAL_memcpy(IeeeOuiByte, &IeeeOui, sizeof(UINT32));
