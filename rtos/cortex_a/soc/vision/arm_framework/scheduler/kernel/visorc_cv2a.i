...
#ifndef VISORC_CV2A_I_FILE
#define VISORC_CV2A_I_FILE
...
uint32_t krn_visorc_init(void)
{
    ...
    krn_module_printU5("[AMBACV] TARGET CHIP   :   CV2A \n",0U,0U,0U,0U,0U);
    return retcode;
}
...
uint32_t krn_visorc_start(uint32_t flag, visorc_init_params_t *sysconfig)
{
    ...
    if(sysconfig == NULL) {
        ...
        if(retcode == ERRCODE_NONE) {
            ...
            /* To be consistent with CV2A although it is not necessary. */
