...
{
    if (index == 1 && (app_helper.check_lte_left_connected() == 0 && app_helper.check_lte_right_connected() == 0)) {
        return 0;
    }
    return 1;
}
        ...
        if (app_helper.linux_booted && app_helper.check_file_exist(mcu_update_infos[index].path, &filesize)) {
            ...
            if (pFile) {
                ...
                for (i = 0; i < sizeof(buf) / 64; i++) {
                    ...
                    check_sum += page_num;
                    if (mcu_update_debug) {
                        debug_line("write mcu: %d, page: 0x%.2x, %d", index, page_num, page_num * 100 / mcu_update_infos[index].end_page);
                    }
                    i2c_write_reg(mcu_update_infos[index].i2c_channel, mcu_update_infos[index].slave_addr, base_reg++, page_num++);
                    for (j = 0; j < 32; j++) {
                    ...
                    if (page_num >= mcu_update_infos[index].end_page) {
                        break;
                    }
                    if (check_lte_mcu_update(index) == 0) {
                        break;
