...
#define TEST_CASE_STR "rtsl_otp_read_aes_key"

static void test_fail_001(void)
{
    UINT32 uret;

    uret = AmbaRTSL_OtpAesKeyRead(NULL, 512, 0);
...
static void test_fail_002(void)
{
    ...
    uret = AmbaRTSL_OtpAesKeyRead(buffer, 5, 0);
...
static void test_fail_003(void)
{
    ...
    uret = AmbaRTSL_OtpAesKeyRead(buffer, 32, 5);
...
static void test_fail_004(void)
{
    ...
    uret = AmbaRTSL_OtpAesKeyRead(buffer, 32, 0);

    flag_otp_engine_running = 1;

    test_value_verify(uret, RTSL_ERR_OTP_WAIT_TIMEOUT, TEST_CASE_STR, 4);
