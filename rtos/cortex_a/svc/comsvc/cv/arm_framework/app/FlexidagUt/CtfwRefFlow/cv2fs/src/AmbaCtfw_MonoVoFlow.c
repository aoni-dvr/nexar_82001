...
void AmbaCtfw_MonoVoProfilerReport(void)
{
    ...
    (void) Ctfw_ProfilerTimerReport(&gCtfwMonoVoTaskHandle, 0U);
...
UINT32 AmbaCtfw_MonoVoFeedOneFrame(const CtfwFrame_t *pFrame)
{
    ...
    if ( pFrame == NULL ) {
        ArmLog_ERR(FLOW_NAME, "AmbaCtfw_MonoVoaFeedOneFrame sanity check fail", 0U, 0U);
        Rval = ARM_NG;
    }
...
UINT32 AmbaCtfw_MonoVoProduceFrame(const AMBA_CV_IMG_BUF_s *pImage, UINT32 FrameIndex, CtfwFrame_t *pFrame)
{
    ...
    if (pImage == NULL) {
        ArmLog_ERR(FLOW_NAME, "## AmbaCtfw_StixelV2ProduceFrame(), pImage == NULL", 0, 0);
        Rval = ARM_NG;
    } else {
        flexidag_memblk_t *pInputData = NULL;
        CtfwFrameMetadata_t *pMonoVoInMeta = NULL;
        ...
        (void) AmbaWrap_memcpy((void*)&pMonoVoInMeta, (void*)&pFrame->MetaData.pBuffer, sizeof(void*));
        pMonoVoInMeta->FrameIndex = FrameIndex;
        Rval = (UINT32) AmbaKAL_GetSysTickCount(&pMonoVoInMeta->TimeStamp);
    }
                           ...
                           CtfwGetProcResult_t GetResultCb, flexidag_memblk_t FlexiBinBlk, UINT32 PoolFrameNumTobeAlloc, CtfwFrame_t FramePool[])
{
    ...
    UINT32 CVMemSize = 0U;
    UINT32 InputDataSize = (UINT32) sizeof(AMBA_CV_IMG_BUF_s);
    UINT32 OutBuffDepth = CTFW_MAX_BUFFER_DEPTH;

    ArmLog_DBG(FLOW_NAME, "## Hello AmbaCtfw_MonoVoaInit!!", 0U, 0U);

    if ( (pSpufexCfg == NULL) || (pMonoVoCfg == NULL) || (GetResultCb == NULL) || (gTotalBuf.pBuffer != NULL)) {
        ArmLog_ERR(FLOW_NAME, "AmbaCtfw_MonoVoaInit sanity check fail", 0U, 0U);
        Rval = ARM_NG;
    }

    if ( PoolFrameNumTobeAlloc < CTFW_MAX_BUFFER_DEPTH ) {
        ArmLog_ERR(FLOW_NAME, "AmbaCtfw_MonoVoaInit fail, PoolFrameNumTobeAlloc(%u) < CTFW_MAX_BUFFER_DEPTH(%u)",
    ...
    if (Rval == ARM_OK) {
        ...
        Rval |= Ctfw_TaskQuerySize(&gCtfwMonoVoTaskObj, OutBuffDepth, &TskReqSize);
        CVMemSize += TskReqSize;
    ...
    if (Rval == ARM_OK) {
        ...
        Rval |= Ctfw_TaskInit(&gCtfwMonoVoTaskObj,  NULL,           pMonoVoCfg, NULL,           FLOW_NAME, gMemPoolId, OutBuffDepth, &gCtfwMonoVoTaskHandle);
        Rval |= Ctfw_TaskInit(&gCtfwSinkTaskObj,    GetResultCb,    NULL,       NULL,           FLOW_NAME, gMemPoolId, OutBuffDepth, &gCtfwSinkTaskHandle);
    }

    if (Rval == ARM_OK) {
        Rval |= AmbaCtfw_MonoVoStart();
    }
    ...
    if (Rval == ARM_OK) {
        ...
        Rval |= Ctfw_TaskConnect_TF(&gCtfwOffexCv2aTaskHandle, &gCtfwMonoVoTaskHandle);
        (void) AmbaKAL_TaskSleep(100U);
        Rval |= Ctfw_TaskConnect_TF(&gCtfwMonoVoTaskHandle, &gCtfwSinkTaskHandle);
        (void) AmbaKAL_TaskSleep(100U);
    }
    ...
    if (Rval == ARM_OK) {
        ...
        (void) Ctfw_TaskCheckConnection(&gCtfwMonoVoTaskHandle);
...
UINT32 AmbaCtfw_MonoVoStop(void)
{
    ...
    Rval |= Ctfw_TaskStop(&gCtfwMonoVoTaskHandle);
...
UINT32 AmbaCtfw_MonoVoDeInit(void)
{
    ...
    Rval |= Ctfw_TaskDeinit(gMemPoolId, &gCtfwMonoVoTaskHandle);
...
UINT32 AmbaCtfw_MonoVoStart(void)
{
    ...
    Rval |= Ctfw_TaskStart(&gCtfwMonoVoTaskHandle);
