...
errcode_enum_t  cvtask_stereo_throttler_register(cvtask_entry_t *pCVTaskEntry)
{
    ...
    pCVTaskEntry->cvtask_type             = CVTASK_TYPE_FEX;
...
#define STEREO_CV2           0x09U

#define STEREO_CV2AQ99       0x00U
#define STEREO_CV2AQ75       0xC5U
#define STEREO_CV2AX85       0x85U
#define STEREO_CV2U85        0x65U

#define STEREO_CV2AQ99_A1    0x00U
#define STEREO_CV2AQ75_A1    0x23U
#define STEREO_CV2AX85_A1    0x53U
#define STEREO_CV2U85_A1     0x93U

                pCVTaskMemory->output[0].buffer_size = 0;
                if(AmbaUUID[1] == STEREO_CV2) {
                    if((AmbaUUID[0] == STEREO_CV2AQ99) || (AmbaUUID[0] == STEREO_CV2AQ75) || (AmbaUUID[0] == STEREO_CV2AX85) || (AmbaUUID[0] == STEREO_CV2U85)
                       || (AmbaUUID[0] == STEREO_CV2AQ99_A1) || (AmbaUUID[0] == STEREO_CV2AQ75_A1) || (AmbaUUID[0] == STEREO_CV2AX85_A1) || (AmbaUUID[0] == STEREO_CV2U85_A1)) {
                        if(pCVTaskMemory->input[0].io_name[0] == STEREO_IDSP_PICINFO) { //"IDSP_PICINFO" for spu
                        ...
                        } else if(pCVTaskMemory->input[0].io_name[0] == STEREO_FEX_FEATURES_LIST) {   //"FEX_FEATURES_LIST" for fex
                            pCVTaskMemory->output[0].buffer_size = sizeof(AMBA_CV_FEX_DATA_s);
                        }
                    }
                }
    ...
    if (pCVTaskParams == NULL) {
        ...
        if (pCVTaskParams->vpCMEM_temporary_scratchpad == NULL) {
            ...
            if (is_not_err(retcode)) {
                ...
                cmem_addr = (uint32_t)pCVTaskParams->vpCMEM_temporary_scratchpad;

                //if(AmbaUUID[1] == 0x09U){
                while (left > 0) {
                    xfersize = (left > 0x400) ? 0x400 : left;
                    dram_2_cmem(cmem_addr, src_daddr, xfersize>>2);
                    cmem_2_dram(dst_daddr, cmem_addr, xfersize>>2);
                    src_daddr  += xfersize;
                    dst_daddr  += xfersize;
                    left       -= xfersize;
                } /* while (left > 0) */
