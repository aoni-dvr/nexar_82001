...
//---------------------
    int per_tile_size = 4;
    int num_of_tiles_per_cache = 128;
    int dma_xfer_size = per_tile_size * num_of_tiles_per_cache;//     4*128   = 512
    int num_of_itr = sizeof(AMBA_IK_COLOR_CORRECTION_s) / dma_xfer_size;//    .../512

    uint32_t da_0;
    uint32_t *word_ptr0[2], *wptr_0;

    da_0 =  (uint32_t)inputdata->pCC0;
    parray = inputdata->pUINT32Array;
    piarray = inputdata->pINT32Array;
    CVTASK_PRINTF(LVL_CRITICAL, "[adj ] : %d, %d, %d, %d, %d, \n",
                  *parray, *(parray+3), *(parray+5), *(parray+7), parray);
    ...
    CVTASK_PRINTF(LVL_CRITICAL, "[adji] : %d, %d, %d, %d, %d, \n",
                  *piarray, *(piarray+1), *(piarray+2), *(piarray+3), piarray);

    CVTASK_PRINTF(LVL_CRITICAL, "[adj pipe0] : %d, %d, %d, %d, %d, \n",
                  inputdata->pPipe0->BlackCorr.BlackB, inputdata->pPipe0->BlackCorr.BlackR, inputdata->pPipe0->BlackCorr.BlackGr, inputdata->pPipe0->BlackCorr.BlackGb, 0);

    CVTASK_PRINTF(LVL_CRITICAL, "[adj pipe1] : %d, %d, %d, %d, %d, \n",
                  inputdata->pPipe1->BlackCorr.BlackB, inputdata->pPipe1->BlackCorr.BlackR, inputdata->pPipe1->BlackCorr.BlackGr, inputdata->pPipe1->BlackCorr.BlackGb, 0);

    CVTASK_PRINTF(LVL_CRITICAL, "[adj pipe2] : %d, %d, %d, %d, %d, \n",
                  inputdata->pPipe2->BlackCorr.BlackB, inputdata->pPipe2->BlackCorr.BlackR, inputdata->pPipe2->BlackCorr.BlackGr, inputdata->pPipe2->BlackCorr.BlackGb, 0);

    start_time = get_cur_time(); // take the start audio clock tick
    word_ptr0[0] = (uint32_t*)ccb->cache[0];
    word_ptr0[1] = (uint32_t*)ccb->cache[1];
    visdma_dram_2_cmem(ccb->cache[0], da_0, (dma_xfer_size >> 2));
    da_0 += dma_xfer_size;
    visdma_dram_2_cmem_nowait(ccb->cache[1], da_0, (dma_xfer_size >> 2));

    for (i=0; i<num_of_itr; i++) { // loop through cache lines
        for (j=0; j<num_of_tiles_per_cache; j++) {
        ...
        wait_vis_w_dram_dma(); // wait for last nowait DMA to finish
        // kick off another DMA transfer
        visdma_dram_2_cmem_nowait(ccb->cache[ping_pong], da_0, ((dma_xfer_size) >> 2));
        ping_pong = !ping_pong; // point to the other ping-pong buffer, which should be ready now
    }

    end_time = get_cur_time();
    CVTASK_PRINTF(LVL_CRITICAL, "[adj_get data_Time] :%10u->%10u  total:%10u\n",
                  start_time, end_time, (end_time-start_time), 0, 0);
    ...
    for (i=0; i<num_of_itr; i++) { // loop through cache lines
        for (j=0; j<num_of_tiles_per_cache; j++) {
            if(((scb->temp1[j]>=scb->temp0[j]) && (scb->temp2[j]>=scb->temp1[j])) || ((scb->temp1[j]<=scb->temp0[j]) && (scb->temp2[j]<=scb->temp1[j]))) {
                error_code = 0;
            } else {
                error_code = 1;
            }
    ...
    end_time = get_cur_time();
    CVTASK_PRINTF(LVL_CRITICAL, "[adj_compare data_Time] :%10u->%10u  total:%10u, k= %d\n",
                  start_time, end_time, (end_time-start_time), k, 0);


    // // if(inputdata->pPipe0->CEInfoUpdate == 1U){
    //   ce_check(ccb, scb,inputdata);
    // // }
    //   tonecurve_check(ccb, scb,inputdata);
    //   cc_check(ccb, scb,inputdata);
    //   dbp_check(ccb, scb,inputdata);
    all_filter_check(ccb, scb,inputdata);
...
errcode_enum_t awb_redundancy_func(awb_sim_ccb_t *ccb, awb_sim_scb_t *scb, AMBA_AAA_Flexidag_INFO_t *inputdata)
{
    ...
    if((ccb->dbg_prt==1U) || (ccb->dbg_prt==99U) || (ccb->dbg_prt==2U) ) {
        CVTASK_PRINTF(LVL_CRITICAL, "\n[cvtask_vptask_awb_run] adj ratio , scb->GainR:  %d,  scb->GainB: %d,",
    ...
    if((ccb->dbg_prt==1U) ||(ccb->dbg_prt==99U) || (ccb->dbg_prt==2U) ) {
        CVTASK_PRINTF(LVL_CRITICAL, "\n[cvtask_vptask_awb_run] refine , scb->GainR:  %d,  scb->GainB: %d,",
                      scb->GainR_Res, scb->GainB_Res, 0, 0, 0);
    }
    ...
    if((ccb->dbg_prt==1U) ||(ccb->dbg_prt==99U) || (ccb->dbg_prt==2U) ) {
        CVTASK_PRINTF(LVL_CRITICAL, "\n[cvtask_vptask_awb_run] results , scb->GainR:  %d,  scb->GainB: %d,",
                      scb->GainR_Res, scb->GainB_Res, 0, 0, 0);
    }
...
errcode_enum_t  cvtask_vptask_awb_run(const cvtask_parameter_interface_t *pCVTaskParams)
{
    ...
    if (pCVTaskParams->CMEM_temporary_scratchpad_size >= 2560) { // should always be 4096 bytes
        ...
        adj_boundary_check(ccb, &scb,inputdata);
        ...
        output->AwbResult.CmpResult = scb.Cmp_Result;
