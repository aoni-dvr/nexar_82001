...
DRAM_TYPE_TABLE = {1: "DDR3", 4: "MDDR3", 5: "DDR4", 6: "LPDDR4"}

ASM_TEMPLATE = {"LPDDR4": "/lib/CV25_LPDDR4_temp.asm"}
...
H_TO_ASM_MAP = {
    ...
    # n-to-1 mapping
    # ---------- DDRC0 ----------
    "DDRIO_DLL0_DDRC0_VAL": ["DLL_Setting0Param"],
    "DDRIO_DLL1_DDRC0_VAL": ["DLL_Setting1Param"],
    "DDRIO_DLL2_DDRC0_VAL": ["DLL_Setting2Param"],
    "DDRIO_DLL3_DDRC0_VAL": ["DLL_Setting3Param"],
    ...
    # ---------- DDRC1 ----------
    "DDRIO_DLL0_DDRC1_VAL": ["DLL_Setting0Param"],
    "DDRIO_DLL1_DDRC1_VAL": ["DLL_Setting1Param"],
    "DDRIO_DLL2_DDRC1_VAL": ["DLL_Setting2Param"],
    "DDRIO_DLL3_DDRC1_VAL": ["DLL_Setting3Param"],
    ...
    "DRAM_PAD_TERM_DDRC0_VAL": ["DRAM_PadTermParam"],
    "DRAM_PAD_TERM2_DDRC0_VAL": ["DRAM_PadTerm2Param"],
    "DRAM_PAD_TERM3_DDRC0_VAL": ["DRAM_PadTerm3Param"],
    ...
    "DRAM_PAD_TERM_DDRC1_VAL": ["DRAM_PadTermParam"],
    "DRAM_PAD_TERM2_DDRC1_VAL": ["DRAM_PadTerm2Param"],
    "DRAM_PAD_TERM3_DDRC1_VAL": ["DRAM_PadTerm3Param"],
    "DRAM_BYTE_MAP_DDRC1_VAL": ["DRAM_ByteMapParam"],

    # n-to-1 mapping
    # ---------- DDRC0 ----------
    "LPDDR4_DQ_CA_VREF_DDRC0_VAL": ["DRAM_lpddr4DqCaVrefParam"],
    "LPDDR4_DQ_WRITE_DLY_DDRC0_VAL": ["DRAM_lpddr4DqWriteDlyParam"],
    "LPDDR4_DQ_READ_DLY_DDRC0_VAL": ["DRAM_lpddr4DqReadDlyParam"],
    "LPDDR4_DQS_WRITE_DLY_DDRC0_VAL": ["DRAM_lpddr4DqsWriteDlyParam"],
    "LPDDR4_TRAIN_MPC_RDLY_DDRC0_VAL": ["DRAM_lpddr4TrainMpcRdlyParam"],
    ...
    # ---------- DDRC1 ----------
    "LPDDR4_DQ_CA_VREF_DDRC1_VAL": ["DRAM_lpddr4DqCaVrefParam"],
    "LPDDR4_DQ_WRITE_DLY_DDRC1_VAL": ["DRAM_lpddr4DqWriteDlyParam"],
    "LPDDR4_DQ_READ_DLY_DDRC1_VAL": ["DRAM_lpddr4DqReadDlyParam"],
    "LPDDR4_DQS_WRITE_DLY_DDRC1_VAL": ["DRAM_lpddr4DqsWriteDlyParam"],
    "LPDDR4_TRAIN_MPC_RDLY_DDRC1_VAL": ["DRAM_lpddr4TrainMpcRdlyParam"],
    ...
    "LPDDR4_MR3_VAL": ["LPDDR4_ModeReg3ParamFsp0", "LPDDR4_ModeReg3ParamFsp1"],
    "LPDDR4_MR11_VAL": ["LPDDR4_ModeReg11ParamFsp0", "LPDDR4_ModeReg11ParamFsp1"],
    "LPDDR4_MR12_VAL": ["LPDDR4_ModeReg12ParamFsp0", "LPDDR4_ModeReg12ParamFsp1"],
    "LPDDR4_MR13_VAL": ["LPDDR4_ModeReg13ParamFsp0"],
    "LPDDR4_MR14_VAL": ["LPDDR4_ModeReg14ParamFsp0", "LPDDR4_ModeReg14ParamFsp1"],
    "LPDDR4_MR22_VAL": ["LPDDR4_ModeReg22ParamFsp0", "LPDDR4_ModeReg22ParamFsp1"],
}

def version():
    return "parser_cv25: %s" % __version__
...
def parse(out, d, platform):
    ...
    # ========== Find dq r/w dly ==========
    k = "DRAM_lpddr4DqReadDlyParam"
    reg_name, reg_raw = plat.collect_from_target(k, d, None, True)
    reg = Register(reg_name, reg_raw, out, True,
        #Bitfield("DDRC_DQ_READ_DELAY", (4, 0), lambda x: (x >> 1) & 0xf if (x & 0x1 == 1) else 0)
        Bitfield("DDRC_DQ_READ_DELAY", (4, 0), None)   # bstiniparser would take care of en bit
    )

    k = "DRAM_lpddr4DqWriteDlyParam"
    reg_name, reg_raw = plat.collect_from_target(k, d, None, True)
    reg = Register(reg_name, reg_raw, out, True,
        Bitfield("DDRC_DQ_WRITE_DELAY", (6, 0), None)
    )

    # ========== Find dq vref ==========
    k = "DRAM_lpddr4DqCaVrefParam"
    reg_name, reg_raw = plat.collect_from_target(k, d, None, True)
    reg = Register(reg_name, reg_raw, out, True,
        #Bitfield("DDRC_DQ_VREF", (27, 21), None),
        #Bitfield("DDRC_CA_VREF", (14, 10), None),
        Bitfield("DDRC_DQ_CA_VREF", (27, 21), None) # use only DQ ch B
    )
    ...
    reg = Register(reg_name, reg_raw, None, True,
        Bitfield("DDRC_NOTERM", (0, 0), None),
        Bitfield("DDRC_TERM", (7, 5), None),
        Bitfield("DDRC_DDS", (4, 2), None)
    )
    dq_noterm = reg.bitfield["DDRC_NOTERM"].value()
    dq_term_val = reg.bitfield["DDRC_TERM"].value()
    dq_dds_val = reg.bitfield["DDRC_DDS"].value()
    ...
    k = "DRAM_PadTerm2Param"
    reg_name, reg_raw = plat.collect_from_target(k, d, None, True)
    reg = Register(reg_name, reg_raw, None, True,
        Bitfield("DDRC_PDDS", (10, 8), None)
    )
    dq_pdds_val = reg.bitfield["DDRC_PDDS"].value()
    dq_pdds.update({dq_upper: dq_pdds_val})                         # DQ[31:16] of PDDS

    k = "DRAM_PadTerm3Param"
    reg_name, reg_raw = plat.collect_from_target(k, d, None, True)
    reg = Register(reg_name, reg_raw, None, True,
        Bitfield("DDRC_NOTERM", (15, 15), None),
        Bitfield("DDRC_TERM", (19, 17), None),
        Bitfield("DDRC_DDS", (14, 12), None),
        Bitfield("DDRC_PDDS", (22, 20), None)
    )
    dq_noterm = reg.bitfield["DDRC_NOTERM"].value()
    dq_term_val = reg.bitfield["DDRC_TERM"].value()
    dq_dds_val = reg.bitfield["DDRC_DDS"].value()
    ...
    if not plat.all_equal([r for r in dq_term.values()]):
        print("[Warning] DQ_TERM is not identical on all DQ! ", end='')
        for r, v in dq_term.items():
            print("%s:%d " % (r, v), end='')
        print()

    if not plat.all_equal([r for r in dq_dds.values()]):
        print("[Warning] DQ_DDS is not identical on all DQ! ", end='')
        for r, v in dq_dds.items():
            print("%s:%d " % (r, v), end='')
        print()

    if not plat.all_equal([r for r in dq_pdds.values()]):
        print("[Warning] DQ_PDDS is not identical on all DQ! ", end='')
        for r, v in dq_pdds.items():
            print("%s:%d " % (r, v), end='')
        print()

    out.update({"DDRC_TERM": "%s" % plat.DRIVE_STRENGTH[dq_term[dq_upper]]})
    out.update({"DDRC_DDS": "%s" % plat.DRIVE_STRENGTH[dq_dds[dq_upper]]})
    out.update({"DDRC_PDDS": "%s" % plat.DRIVE_STRENGTH[dq_pdds[dq_upper]]})



    if (dram_type == "LPDDR4"):
        # ========== Find DRAM_DS ==========
        k = "LPDDR4_ModeReg3ParamFsp0"
        reg_name, reg_raw = plat.collect_from_target(k, d, None, True)
        reg = Register(reg_name, reg_raw, out, True,
