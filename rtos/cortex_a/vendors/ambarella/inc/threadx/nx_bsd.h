...
/*  San Diego, CA  92127                    www.expresslogic.com          */
/*                                                                        */
/**************************************************************************/
...
#define NX_BSD_RECEIVE_EVENT                ((UINT32) 0x00000001U)    /* Event flag to signal a receive packet event                          */
#define NX_BSD_SELECT_EVENT                 ((UINT32) 0x00008000U)    /* Event flag to signal a thread is waiting in select                   */
#define NX_BSD_ALL_EVENTS                   ((UINT32) 0xFFFFFFFFU)    /* All event flag                                                       */
#define NX_BSD_CONNECT_EVENT                ((UINT32) 0x00000002U)
#define NX_BSD_LINGER_EVENT                 ((UINT32) 0x00000004U)    /* Event flag to signal a timed linger state has expired on a socket    */
#define NX_BSD_TIMED_WAIT_EVENT             ((UINT32) 0x00000008U)    /* Event flag to signal a timed wait state has expired on a socket      */
#define NX_BSD_TIMER_EVENT                  ((UINT32) 0x00000010U)    /* Event flag to singal a BSD 1 sec timer */
...
#define     INADDR_ANY                      0
#define     NX_BSD_LOCAL_IF_INADDR_ANY      0xFFFFFFFFU
...
#if 1 // From Ambarella for using newlib. NOTE: some errors are different.
#include <errno.h>
...
#endif // From Ambarella for using newlib. NOTE: some errors are different.
#else
//for RTSP
#ifndef ENOTCONN
#define ENOTCONN 128        /* Socket is not connected */
#endif
#endif  //#ifndef NX_PATCH_MISRA
...
typedef struct NX_BSD_SOCKET_STRUCT
{
    ...
    UINT32               nx_bsd_socket_source_ip_address;
    UINT32               nx_bsd_socket_peer_ip;
...
INT  bsd_initialize(NX_IP *default_ip, NX_PACKET_POOL *default_pool, CHAR *bsd_thread_stack_area, UINT32 bsd_thread_stack_size, UINT bsd_thread_priority);
...
#ifndef NX_PATCH_MISRA // From Ambarella, we include errno.h for using newlib, but netx use it's define for global variable.
