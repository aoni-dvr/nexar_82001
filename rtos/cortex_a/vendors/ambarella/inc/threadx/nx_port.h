...
/*                                                                        */
/*  12-12-2012     William E. Lamie         Initial SMP/Cortex-A8 Support */
/*                                            for IAR Version 5.0         */
/*                                                                        */
/**************************************************************************/
...
#define NX_CALLER_CHECKING_EXTERNS          extern  TX_THREAD           _tx_timer_thread;

#define NX_THREADS_ONLY_CALLER_CHECKING     if ((_tx_thread_smp_current_state_get()) || \
                                                (_tx_thread_smp_current_thread_get() == TX_NULL) || \
                                                (_tx_thread_smp_current_thread_get() == &_tx_timer_thread)) \
                                                return(NX_CALLER_ERROR);

#define NX_INIT_AND_THREADS_CALLER_CHECKING if (((_tx_thread_smp_current_state_get()) && (_tx_thread_smp_current_state_get() < ((UINT32) 0xF0F0F0F0))) || \
                                                (_tx_thread_smp_current_thread_get() == &_tx_timer_thread)) \
                                                return(NX_CALLER_ERROR);


#define NX_NOT_ISR_CALLER_CHECKING          if ((_tx_thread_smp_current_state_get()) && (_tx_thread_smp_current_state_get() < ((UINT32) 0xF0F0F0F0))) \
                                                return(NX_CALLER_ERROR);

#define NX_THREAD_WAIT_CALLER_CHECKING      if ((wait_option) && \
                                               ((_tx_thread_smp_current_thread_get() == NX_NULL) || (_tx_thread_smp_current_state_get()) || (_tx_thread_smp_current_thread_get() == &_tx_timer_thread))) \
                                            return(NX_CALLER_ERROR);
...
#define NX_THREAD_WAIT_CALLER_CHECKING      if ((wait_option) && \
                                               ((_tx_thread_smp_current_thread_get() == NX_NULL) || (_tx_thread_smp_current_state_get()))) \
                                            return(NX_CALLER_ERROR);
