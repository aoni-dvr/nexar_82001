...
UINT    _ux_host_stack_class_register(UCHAR *class_name,
                                      UINT (*class_entry_function)(struct UX_HOST_CLASS_COMMAND_STRUCT *command));
...
UINT    _ux_host_stack_hcd_register(UCHAR *hcd_name,
                                    UINT (*hcd_init_function)(struct UX_HCD_STRUCT *hcd), ULONG hcd_param1, ULONG hcd_param2);
VOID    _ux_host_stack_hcd_thread_entry(ULONG input);
UINT    _ux_host_stack_hcd_transfer_request(UX_TRANSFER *transfer_request);
UINT    _ux_host_stack_initialize(UINT (*ux_system_host_change_function)(ULONG command, UX_HOST_CLASS *host_class, const VOID *arg));
UINT    _ux_host_stack_destroy(void);
UINT    _ux_host_stack_interface_endpoint_get(UX_INTERFACE *intf, UINT endpoint_index, UX_ENDPOINT **endpoint);
UINT    _ux_host_stack_interface_set(UX_INTERFACE *intf);
UINT    _ux_host_stack_interface_setting_select(UX_INTERFACE *intf);
...
UINT    _ux_host_stack_new_device_create(UX_HCD *hcd, UX_DEVICE *device_owner,
        ...
        UINT port_max_power);
UINT    _ux_host_stack_new_endpoint_create(UX_INTERFACE *intf, UCHAR * interface_endpoint);
...
extern UINT  _ux_host_stack_configuration_interface_get(UX_CONFIGURATION *configuration, UINT interface_index, UINT alternate_setting_index, UX_INTERFACE **interface);
extern UINT  uxh_stk_dev_config_get(UX_DEVICE *device, UINT configuration_index, UX_CONFIGURATION **configuration);
extern UINT  uxh_stk_dev_config_select(UX_CONFIGURATION *configuration);
extern UINT  uxh_stk_transfer_request_abort(UX_TRANSFER *transfer_request);
extern UINT  uxh_stk_transfer_request_unlink(UX_TRANSFER *root_request, UX_TRANSFER *remove_request);
extern UINT  uxh_stk_transfer_request_link(UX_TRANSFER *root_request, UX_TRANSFER *link_request);
