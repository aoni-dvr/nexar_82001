...
static mem_short_cb mem_short_notify;
static uint8_t smallStorage[0x2000]; //5K interal buffer to avoid memory shortage
static uint32_t maxShortSize = 0U;
#endif
    ...
    for(uint8_t i = 0; i < LV_MEM_BUF_MAX_NUM; i++) {
        if(LV_GC_ROOT(lv_mem_buf[i]).used == 0) {
            ...
            if(buf == NULL){
                buf = &smallStorage[0];
                mem_short_notify();
                maxShortSize = size > maxShortSize ? size : maxShortSize;
                LV_LOG_ERROR("Out of memory, Max:[0x%X], need size:[0x%X]", maxShortSize, size);
                return buf;
            }
#endif
...
#endif
#ifdef AMBA_CUSTOMIZED
void lv_mem_hookCB(mem_short_cb pCallback)
{
    mem_short_notify = pCallback;
}
#endif
