 ...
 * should be lower than 400k
 */
#define SDMMC_CLK_DEFAULT     400000
#define SDMMC_CLK_BASE        50000000
#define SDMMC_MAX_FREQ        200000000
#define SDMMC_MIN_FREQ        97751
#define SDMMC_MAX_DIV_V2      256
#define SDMMC_MAX_DIV_V3      2046
#define SDMMC_VERSION_3

/*
 * SDMMC card power up wait loop
 */
#define POWER_UP_WAIT               80
...
#define ADMA2_NOP      (0 << 4)    // no OP, go to the next descriptor
#define ADMA2_TRAN     (2 << 4)    // transfer data
#define ADMA2_LINK     (3 << 4)    // link to another descriptor

// 32 bit ADMA descriptor definition
typedef struct _amba_adma32_t {
    uint16_t    attr;
    uint16_t    len;
    uint32_t    addr;
} amba_adma32_t;

typedef struct amba_adma_ext
{
    amba_adma32_t    *adma_des;
} amba_adma_ext_t;
 ...
 * SDMMC structure
 */
typedef struct amba_sdmmc {
    unsigned sdmmc_pbase; // base address
    amba_adma_ext_t    *adma_ext;// Specific DMA handler
 ...
 * SDMMC functions
 */
extern int sdmmc_read (amba_sdmmc_t *sdmmc, void *buf, unsigned blkno, unsigned blkcnt);
extern int sdmmc_init_ctrl(amba_sdmmc_t *sdmmc);
extern int sdmmc_init_card(amba_sdmmc_t *sdmmc);
extern int sdmmc_fini(amba_sdmmc_t *sdmmc);
