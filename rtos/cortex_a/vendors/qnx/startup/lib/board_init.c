...
{
#if !defined(CONFIG_XEN_QNX_QEMU_VIRT)
    /*
     * put any board specific init (such as checking for arguments
     * passed from IPL here.
     */
#if !defined(CONFIG_SOC_CV2FS)
    // CV2FS Cortex A can't access AXI bus, otherwise ARM SError interrupt occurs.
    unsigned int * pNIC400 = (unsigned int *) 0xf1000000;
